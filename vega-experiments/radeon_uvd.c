                                     ...
                                     struct pipe_h265_picture_desc *pic)
{
      ...
      /* Evict stale pictures from our render list */
      for (i = 0; i < 16; ++i) {
            bool is_referenced = false;
            if (!dec->render_pic_list[i])
                  continue;

            for (j = 0; j < 16 && pic->ref[j]; ++j) {
      ...
      /* Find a free slot for the new target picture */
      result.curr_idx = 16;
      for (i = 0; i < 16; i++) {
      ...
      /* This should never happen with a spec-compliant stream and correct DPB management */
      assert(result.curr_idx < 16);
...
static uint32_t get_ref_pic_idx(struct ruvd_decoder *dec, struct pipe_video_buffer *ref)
{
      ...
      uint32_t frame = (uintptr_t)vl_video_buffer_get_associated_data(ref, &dec->base);
                                  ...
                                  const unsigned *restrict sizes)
{
      ...
      if (UNLIKELY(total_bs_size > buf->res->buf->size)) {
            dec->ws->buffer_unmap(dec->ws, buf->res->buf);
            dec->bs_ptr = NULL;
