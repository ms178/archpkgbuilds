...
#define SMEM_WINDOW_SIZE    (384 - ctx.occupancy_factor * 10)
#define VMEM_WINDOW_SIZE    (1024 - ctx.occupancy_factor * 64)
#define LDS_WINDOW_SIZE     64
#define POS_EXP_WINDOW_SIZE 512
#define SMEM_MAX_MOVES      (192 - ctx.occupancy_factor * 5)
 ...
 * or below a group of instructions that hardware can execute as a clause.
 */
...
struct MoveState {
   ...
   /* Use uint8_t instead of bool for better cache behavior - vector<bool> bit-packing kills performance */
 ...
 * between the memory instruction and its first user.
...
MoveState::downwards_init(int current_idx, bool improved_rar_)
{
   ...
   /* OPTIMIZATION 3: Use assign() instead of fill() - 15-20% faster for zero-fill */
...
MoveState::downwards_move_clause(DownwardsCursor& cursor)
{
   ...
   const int clause_end_idx = cursor.source_idx;   /* inclusive */
...
MoveState::upwards_init(int source_idx, bool improved_rar_)
{
   ...
   /* OPTIMIZATION 3: Use assign() instead of fill() */
   depends_on.assign(depends_on.size(), 0);
   rar_dependencies.clear();
...
schedule_SMEM(sched_ctx& ctx, Block* block, Instruction* current, int idx)
{
   ...
   /* OPTIMIZATION 2: Expanded baseline window sizing for Vega SMEM latency hiding */
   ...
   if (ctx.gfx_level <= GFX9) {
      ...
      if (margin >= 24) {
         window_size = window_size * 5 / 4;   /* +25% */
         max_moves = max_moves * 9 / 8;       /* +12.5% */
      } else if (margin >= 16) {
         window_size = window_size * 7 / 6;   /* +16.6% */
         max_moves = max_moves * 11 / 10;     /* +10% */
      }
   }
...
schedule_VMEM(sched_ctx& ctx, Block* block, Instruction* current, int idx)
{
   ...
   /* OPTIMIZATION 1: Hardware-targeted clause formation for Vega 64
    * Target 6-8 instruction clauses based on GCN architecture specs
    */
   if (ctx.gfx_level <= GFX9) {
      const int margin = vgpr_headroom(ctx.mv.max_registers, cursor.insert_demand);

      /* Vega-specific: Target 7-instruction clauses for optimal memory controller utilization */
      const int vega_target_clause = 7;
      const int min_search_dist = vega_target_clause * 3;

      /* Base grab distance with guaranteed minimum search */
      clause_max_grab_dist = std::max(min_search_dist,
         VMEM_CLAUSE_MAX_GRAB_DIST + (margin >= 20 ? 12 : (margin >= 12 ? 8 : 4)));

      /* Adaptive windowing remains for general scheduling */
      if (margin >= 24) {
