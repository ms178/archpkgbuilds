...
namespace aco {
      ...
      static const struct debug_control aco_debug_options[] = {
            ...
            init_once()
            {
                  ...
                  #ifndef NDEBUG
                  if (UNLIKELY(!(debug_flags & aco::DEBUG_NO_VALIDATE))) {
                         ...
                         ac_shader_config* config)
            {
                  ...
                  if (UNLIKELY(family == CHIP_UNKNOWN)) {
                        switch (gfx_level) {
                  ...
                  program->dev.lds_encoding_granule = LIKELY(gfx_level >= GFX11 && stage == fragment_fs) ? 1024
                  : LIKELY(gfx_level >= GFX7)                        ? 512
                  : 256;
                  program->dev.lds_alloc_granule = LIKELY(gfx_level >= GFX10_3) ? 1024 : program->dev.lds_encoding_granule;

                  program->dev.lds_limit = LIKELY(gfx_level >= GFX7) ? 65536 : 32768;

                  program->dev.has_16bank_lds = UNLIKELY(family == CHIP_KABINI || family == CHIP_STONEY);

                  program->dev.vgpr_limit = 256;
                  program->dev.physical_vgprs = 256;
                  program->dev.vgpr_alloc_granule = 4;

                  if (LIKELY(gfx_level >= GFX10)) {
                        program->dev.physical_sgprs = 128 * 20;
                        program->dev.sgpr_alloc_granule = 128;
                        program->dev.sgpr_limit = 108;

                        if (UNLIKELY(family == CHIP_NAVI31 || family == CHIP_NAVI32 || family == CHIP_GFX1151 ||
                              gfx_level >= GFX12)) {
                  ...
                  if (UNLIKELY(program->stage == raytracing_cs))
                        program->dev.vgpr_limit = util_align_npot(128, program->dev.vgpr_alloc_granule);

                  program->dev.scratch_alloc_granule = LIKELY(gfx_level >= GFX11) ? 256 : 1024;

                  program->dev.max_waves_per_simd = 10;
                  if (LIKELY(program->gfx_level >= GFX10_3))
                        program->dev.max_waves_per_simd = 16;
                  else if (LIKELY(program->gfx_level == GFX10))
                        program->dev.max_waves_per_simd = 20;
                  else if (UNLIKELY(program->family >= CHIP_POLARIS10 && program->family <= CHIP_VEGAM))
                        program->dev.max_waves_per_simd = 8;

                  program->dev.simd_per_cu = LIKELY(program->gfx_level >= GFX10) ? 2 : 4;
                  ...
                  program->dev.sram_ecc_enabled = UNLIKELY(program->family == CHIP_VEGA20 ||
                  program->family == CHIP_MI100 || program->family == CHIP_MI200 ||
                  program->family == CHIP_GFX940);
                  program->dev.has_fast_fma32 = LIKELY(program->gfx_level >= GFX9);
                  if (UNLIKELY(program->family == CHIP_TAHITI || program->family == CHIP_CARRIZO ||
                        program->family == CHIP_HAWAII))
                        program->dev.has_fast_fma32 = true;
                  program->dev.has_mac_legacy32 = UNLIKELY(program->gfx_level <= GFX7 || program->gfx_level == GFX10);
                  program->dev.has_fmac_legacy32 = LIKELY(program->gfx_level >= GFX10_3 && program->gfx_level < GFX12);

                  program->dev.fused_mad_mix = LIKELY(program->gfx_level >= GFX10);
                  if (UNLIKELY(program->family == CHIP_VEGA12 || program->family == CHIP_VEGA20 ||
                        program->family == CHIP_MI100 || program->family == CHIP_MI200))
                        program->dev.fused_mad_mix = true;

                  if (LIKELY(program->gfx_level >= GFX12)) {
                        program->dev.scratch_global_offset_min = -8388608;
                        program->dev.scratch_global_offset_max = 8388607;
                  } else if (LIKELY(program->gfx_level >= GFX11)) {
                        program->dev.scratch_global_offset_min = -4096;
                        program->dev.scratch_global_offset_max = 4095;
                  } else if (LIKELY(program->gfx_level >= GFX10 || program->gfx_level == GFX8)) {
                        program->dev.scratch_global_offset_min = -2048;
                        program->dev.scratch_global_offset_max = 2047;
                  } else if (LIKELY(program->gfx_level == GFX9)) {
                  ...
                  else if (LIKELY(program->gfx_level >= GFX7))
                        program->dev.smem_offset_max = 0xffffffff;
                  else if (LIKELY(program->gfx_level >= GFX6))
                  ...
                  } else if (LIKELY(program->gfx_level >= GFX10_3)) {
            ...
            is_wait_export_ready(amd_gfx_level gfx_level, const Instruction* instr)
            {
                  ...
                  (LIKELY(gfx_level >= GFX12) ? (instr->salu().imm & wait_event_imm_wait_export_ready_gfx12)
            ...
            {
                  if (UNLIKELY(instr->opcode == aco_opcode::p_pops_gfx9_overlapped_wave_wait_done ||
                        ...
                        } else if (UNLIKELY(instr->opcode == aco_opcode::p_pops_gfx9_ordered_section_done)) {
            ...
            {
                  if (UNLIKELY(!instr->isVALU()))
                        return false;

                  if (UNLIKELY(gfx_level < GFX8 || gfx_level >= GFX11 || instr->isDPP() || instr->isVOP3P()))
                        return false;
                  ...
                  if (instr->isVOP3()) {
                        ...
                        if (UNLIKELY(instr->format == Format::VOP3))
                              return false;
                        if (UNLIKELY(vop3.clamp && instr->isVOPC() && gfx_level != GFX8))
                              return false;
                        if (UNLIKELY(vop3.omod && gfx_level < GFX9))
                              return false;

                        if (UNLIKELY(!pre_ra && instr->definitions.size() >= 2))
                              return false;

                        for (unsigned i = 1; i < instr->operands.size(); i++) {
                              if (UNLIKELY(instr->operands[i].isLiteral()))
                                    return false;
                              if (UNLIKELY(gfx_level < GFX9 && !instr->operands[i].isOfType(RegType::vgpr)))
                                    return false;
                        }
                  }

                  if (LIKELY(!instr->definitions.empty()) && UNLIKELY(instr->definitions[0].bytes() > 4 && !instr->isVOPC()))
                        return false;

                  if (LIKELY(!instr->operands.empty())) {
                        if (UNLIKELY(instr->operands[0].isLiteral()))
                              return false;
                        if (UNLIKELY(gfx_level < GFX9 && !instr->operands[0].isOfType(RegType::vgpr)))
                              return false;
                        if (UNLIKELY(instr->operands[0].bytes() > 4))
                              return false;
                        if (LIKELY(instr->operands.size() > 1) && UNLIKELY(instr->operands[1].bytes() > 4))
                              return false;
                  }
                  ...
                  if (UNLIKELY(gfx_level != GFX8 && is_mac))
                        return false;

                  if (UNLIKELY(!pre_ra && instr->isVOPC() && gfx_level == GFX8))
                        return false;
                  if (UNLIKELY(!pre_ra && instr->operands.size() >= 3 && !is_mac))
                        return false;

                  return LIKELY(instr->opcode != aco_opcode::v_madmk_f32 && instr->opcode != aco_opcode::v_madak_f32 &&
            ...
            can_use_DPP(amd_gfx_level gfx_level, const aco_ptr<Instruction>& instr, bool dpp8)
            {
                  ...
                  if (UNLIKELY(instr->isSDWA() || instr->isVINTERP_INREG()))
                        return false;

                  if (UNLIKELY((instr->format == Format::VOP3 || instr->isVOP3P()) && gfx_level < GFX11))
                        return false;

                  if (UNLIKELY((instr->isVOPC() || instr->definitions.size() > 1) && instr->definitions.back().isFixed() &&
                        instr->definitions.back().physReg() != vcc && gfx_level < GFX11))
                        return false;

                  if (UNLIKELY(instr->operands.size() >= 3 && instr->operands[2].isFixed() &&
                  ...
                  if (instr->isVOP3() && gfx_level < GFX11) {
                        ...
                        if (UNLIKELY(vop3->clamp || vop3->omod))
                              return false;
                        if (UNLIKELY(dpp8))
                              return false;
                  }
                  ...
                  for (unsigned i = 0; i < instr->operands.size(); i++) {
                        if (UNLIKELY(instr->operands[i].isLiteral()))
                              return false;
                        if (UNLIKELY(!instr->operands[i].isOfType(RegType::vgpr) && i < 2))
                              return false;
                  }

                  if (UNLIKELY(instr->writes_exec()))
                        return false;

                  if (instr->isVOP3P()) {
                        return LIKELY(instr->opcode == aco_opcode::v_fma_mix_f32 ||
                  ...
                  if (UNLIKELY(instr->opcode == aco_opcode::v_pk_fmac_f16))
            ...
            convert_to_DPP(amd_gfx_level gfx_level, aco_ptr<Instruction>& instr, bool dpp8)
            {
                  ...
                  if (dpp8) {
                        ...
                        dpp->fetch_inactive = LIKELY(gfx_level >= GFX10);
                  } else {
                  ...
                  if (UNLIKELY((instr->isVOPC() || instr->definitions.size() > 1) && gfx_level < GFX11))
                        instr->definitions.back().setPrecolored(vcc);

                  if (UNLIKELY(instr->operands.size() >= 3 && instr->operands[2].isOfType(RegType::sgpr) &&
                        gfx_level < GFX11))
                        instr->operands[2].setPrecolored(vcc);
            ...
            {
                  if (UNLIKELY(op == aco_opcode::v_mov_b32))
            ...
            bool
            can_use_opsel(amd_gfx_level gfx_level, aco_opcode op, int idx) // idx is operand_index, -1 for dst
            {
                  // VOP3P instructions have their own separate opsel mechanism for packed math.
                  // This function checks for the standard VALU opsel used for sub-dword selection.
                  if (((uint16_t)instr_info.format[static_cast<int>(op)] & (uint16_t)Format::VOP3P))
                        return false;

                  // General VALU op_sel for sub-dword selection is a GFX9+ feature.
                  // Specific 16-bit op_sel (like true16 on GFX11+) also has its own GFX requirements.
                  if (UNLIKELY(gfx_level < GFX9)) {
                        // Only allow GFX11+ true16 specific opsel if gfx_level is less than GFX9
                  ...
                  switch (op) {
                        /* Cases for instructions known to have specific 16-bit opsel behavior from existing aco_ir.cpp */
                        ...
                        case aco_opcode::v_mul_lo_u16_e64:
                              return true; /* These ops are inherently 16-bit or have specific 16-bit opsel modes */

                        case aco_opcode::v_pack_b32_f16:
                        case aco_opcode::v_cvt_pknorm_i16_f16:
                        case aco_opcode::v_cvt_pknorm_u16_f16:
                              return LIKELY(idx != -1); /* opsel applies to sources, not for destination check */

                        case aco_opcode::v_mad_u32_u16:
                        case aco_opcode::v_mad_i32_i16:
                              return LIKELY(idx >= 0 && idx < 2); /* only src0 and src1 are 16-bit for these */

                        case aco_opcode::v_dot2_f16_f16:
                        case aco_opcode::v_dot2_bf16_bf16:
                              return LIKELY(idx == -1 || idx == 2); /* dst and accumulator (src2) are 32-bit */

                        case aco_opcode::v_cndmask_b16:
                              return LIKELY(idx != 2); /* src2 (sgpr sel) is not a 16-bit data operand for opsel */

                        case aco_opcode::v_interp_p10_f16_f32_inreg:
                        case aco_opcode::v_interp_p10_rtz_f16_f32_inreg:
                              return LIKELY(idx == 0 || idx == 2);
                        case aco_opcode::v_interp_p2_f16_f32_inreg:
                        case aco_opcode::v_interp_p2_rtz_f16_f32_inreg:
                              return LIKELY(idx == -1 || idx == 0);

                              /* VOP3 instructions that explicitly do NOT support general sub-dword opsel */
                              /* (Ensure these are defined in aco_opcodes.py if you keep them) */
                              // case aco_opcode::v_cvt_pkh_f16_f32: // REMOVED - Not in provided aco_opcodes.py
                        ...
                        case aco_opcode::v_cvt_pk_u16_u32:
                              ...
                              /* Cases for VINTERP that might be VOP3 encoded */
                              case aco_opcode::v_interp_p1_f32:
                              case aco_opcode::v_interp_p2_f32:
                                    break; // Fall through to generic VOP3A/B check

                                    /* Added instructions for GFX9+ VOP3A sub-dword opsel */
                                    ...
                                    case aco_opcode::v_interp_p2_f16:
                                          ...
                                          /* Legacy opcodes with VOP3A forms (GFX9+ based on Vega ISA) */
                                          ...
                                          default:
                                                break; // Fall through to general checks
                  }

                  /* Default logic:
                   1 . Check for GFX11+ true 16-bit operat*ions if not handled by specific cases above.
                   2. Check for general VOP3A/VOP3B supporting input modifiers for sub-dword selection.
                   */
                  if (gfx_level >= GFX11 && (get_gfx11_true16_mask(op) & BITFIELD_BIT(idx == -1 ? 3 : idx))) {
                        return true;
                  }
                  ...
                  if (((uint16_t)format & (uint16_t)Format::VOP3)) { // This covers VOP3A and VOP3B
                        // VOP3P was already excluded at the top
                  ...
                  return false; // Default to no opsel if no rule matched
            }
            ...
            instr_is_16bit(amd_gfx_level gfx_level, aco_opcode op)
            {
                  ...
                  switch (op) {
                        ...
                        case aco_opcode::v_madmk_f16: return LIKELY(gfx_level >= GFX9);
                        ...
                        case aco_opcode::v_cvt_norm_u16_f16: return LIKELY(gfx_level >= GFX10);
            ...
            get_reduction_identity(ReduceOp op, unsigned idx)
            {
                  switch (op) {
                        ...
                        case fmin64: return LIKELY(idx) ? 0x7ff00000u : 0u;
                        case fmax16: return 0xfc00u;
                        case fmax32: return 0xff800000u;
                        case fmax64: return LIKELY(idx) ? 0xfff00000u : 0u;
            ...
            get_operand_type(aco_ptr<Instruction>& alu, unsigned index)
            {
                  ...
                  if (UNLIKELY(alu->opcode == aco_opcode::v_fma_mix_f32 || alu->opcode == aco_opcode::v_fma_mixlo_f16 ||
            ...
            {
                  if (instr->isVALU()) {
                        return LIKELY(instr->opcode != aco_opcode::v_readlane_b32 &&
                        instr->opcode != aco_opcode::v_readlane_b32_e64 &&
                  ...
                  if (instr->isSALU() || instr->isBranch() || instr->isSMEM() || instr->isBarrier())
                        return UNLIKELY(instr->opcode == aco_opcode::s_cbranch_execz ||
                        ...
                        switch (instr->opcode) {
                              ...
                              case aco_opcode::p_parallelcopy:
                                    for (Definition def : instr->definitions) {
                                          if (LIKELY(def.getTemp().type() == RegType::vgpr))
                                                return true;
                                    }
                              ...
                              case aco_opcode::p_start_linear_vgpr: return LIKELY(instr->operands.size());
                              default: break;
                        }
                  }
            ...
            can_swap_operands(aco_ptr<Instruction>& instr, aco_opcode* new_op, unsigned idx0, unsigned idx1)
            {
                  ...
                  if (UNLIKELY(instr->isDPP()))
                        return false;

                  if (UNLIKELY(!instr->isVOP3() && !instr->isVOP3P() && !instr->operands[0].isOfType(RegType::vgpr)))
                        return false;

                  aco_opcode candidate = get_swapped_opcode(instr->opcode, idx0, idx1);
                  if (UNLIKELY(candidate == aco_opcode::num_opcodes))
                        return false;
            ...
            wait_imm::pack(enum amd_gfx_level gfx_level) const
            {
                  ...
                  if (UNLIKELY(gfx_level < GFX9 && vm == wait_imm::unset_counter))
                        imm |= 0xc000;
                  if (UNLIKELY(gfx_level < GFX10 && lgkm == wait_imm::unset_counter))
                        imm |= 0x3000;
                  return imm;
            }
            ...
            {
                  wait_imm imm;
                  imm.vm = LIKELY(gfx_level >= GFX9) ? 63 : 15;
                  imm.exp = 7;
                  imm.lgkm = LIKELY(gfx_level >= GFX10) ? 63 : 15;
                  imm.vs = LIKELY(gfx_level >= GFX10) ? 63 : 0;
                  imm.sample = LIKELY(gfx_level >= GFX12) ? 63 : 0;
                  imm.bvh = LIKELY(gfx_level >= GFX12) ? 7 : 0;
                  imm.km = LIKELY(gfx_level >= GFX12) ? 31 : 0;
                  return imm;
            }
            ...
            {
                  if (UNLIKELY(!instr->isSALU() || (!instr->operands.empty() && instr->operands[0].physReg() != sgpr_null)))
                        return false;
                  ...
                  if (op == aco_opcode::s_wait_loadcnt) {
                        ...
                        uint32_t ds_val = packed & 0x3f;
                        vm = std::min<uint8_t>(vm, LIKELY(vm2 != 0x3f) ? vm2 : wait_imm::unset_counter);
                        lgkm = std::min<uint8_t>(lgkm, LIKELY(ds_val != 0x3f) ? ds_val : wait_imm::unset_counter);
                  } else if (op == aco_opcode::s_wait_storecnt_dscnt) {
                        uint32_t vs2 = (packed >> 8) & 0x3f;
                        uint32_t ds_val = packed & 0x3f;
                        vs = std::min<uint8_t>(vs, LIKELY(vs2 != 0x3f) ? vs2 : wait_imm::unset_counter);
                        lgkm = std::min<uint8_t>(lgkm, LIKELY(ds_val != 0x3f) ? ds_val : wait_imm::unset_counter);
                        ...
                        if (UNLIKELY(vm2 == (LIKELY(gfx_level >= GFX9) ? 0x3f : 0xf)))
                              vm2 = wait_imm::unset_counter;
                        if (UNLIKELY(exp2 == 0x7))
                              exp2 = wait_imm::unset_counter;
                        if (UNLIKELY(lgkm2 == (LIKELY(gfx_level >= GFX10) ? 0x3f : 0xf)))
            ...
            wait_imm::build_waitcnt(Builder& bld)
            {
                  ...
                  if (LIKELY(gfx_level >= GFX12)) {
                        ...
                        if (UNLIKELY(vs != wait_imm::unset_counter)) {
            ...
            should_form_clause(const Instruction* a, const Instruction* b)
            {
                  ...
                  if (UNLIKELY((a->isMTBUF() && b->isMUBUF()) || (a->isMUBUF() && b->isMTBUF()))) {
                  } else if (a->format != b->format) {
                        return false;
                  }

                  if (UNLIKELY(a->operands.empty() || b->operands.empty()))
                        return false;
                  ...
                  if (a->isVMEM() || a->isSMEM())
                        return LIKELY(a->operands[0].tempId() == b->operands[0].tempId());
                  ...
                  if (instr->opcode == aco_opcode::v_interp_p2_f32 || instr->opcode == aco_opcode::v_mac_f32 ||
                        ...
                        ops.push_back(2);
                              ...
                              ops.push_back(0);
                                    ...
                                    ops.push_back(2);
                                    } else if (UNLIKELY(instr->opcode == aco_opcode::image_bvh8_intersect_ray)) {
                                          ops.push_back(3 + 2);
                                          ops.push_back(3 + 3);
                                    }
                                    return ops;
            ...
            {
                  if (UNLIKELY(instr->opcode == aco_opcode::image_bvh64_intersect_ray ||
                        instr->opcode == aco_opcode::image_bvh8_intersect_ray)) {
                        return vmem_bvh;
                        } else if (UNLIKELY(gfx_level >= GFX12 && instr->opcode == aco_opcode::image_msaa_load)) {
                              return vmem_sampler;
                        } else if (instr->isMIMG() && !instr->operands[1].isUndefined() &&
                              instr->operands[1].regClass() == s4) {
                              bool point_sample_accel =
                              UNLIKELY(gfx_level == GFX11_5 && (instr->opcode == aco_opcode::image_sample ||
                  ...
                  if (instr->isVMEM() || instr->isFlatLike() || instr->isDS() || instr->isEXP()) {
                        ...
                        for (const Definition& def : instr->definitions) {
                              if (def.regClass().type() == RegType::sgpr) {
                                    res.sa_sdst = 0;
                                    if (UNLIKELY(instr->opcode == aco_opcode::v_readfirstlane_b32))
                                          res.va_exec = 0;
                                    break;
                              }
                        }
                  } else if (UNLIKELY(instr_info.classes[(int)instr->opcode] == instr_class::branch ||
            ...
            {
                  if (UNLIKELY(program->gfx_level < GFX11))
                        return false;

                  if (UNLIKELY(program->gfx_level == GFX11_5 && (program->stage.hw == AC_HW_NEXT_GEN_GEOMETRY_SHADER ||
                  ...
                  Builder bld(program);
                  if (LIKELY(!block.instructions.empty() && block.instructions.back()->opcode == aco_opcode::s_endpgm)) {
            ...
            get_instr_data_size(Format format)
            {
                  switch (format) {
                        ...
                        default:
                              if (UNLIKELY((uint16_t)format & (uint16_t)Format::DPP16))
                                    return sizeof(DPP16_instruction);
                        else if (UNLIKELY((uint16_t)format & (uint16_t)Format::DPP8))
                              return sizeof(DPP8_instruction);
                        else if (UNLIKELY((uint16_t)format & (uint16_t)Format::SDWA))
