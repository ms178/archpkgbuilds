...
for sz in [16, 32, 64]:
    for mulz in ([False, True] if sz == 32 else [False]):
        fmul = 'fmulz(is_only_used_by_fadd)' if mulz else f'fmul@{sz}(is_only_used_by_fadd)'
...
for sz in (16,):
    ...
    ffma = f'ffma@{sz}'
    option = (
        f'(options->float_mul_add{sz} & nir_float_muladd_support_fuse) && '
        f'(options->float_mul_add{sz} & nir_float_muladd_support_has_ffma) && '
        f'(!(options->float_mul_add{sz} & nir_float_muladd_support_has_fmad) || '
        f'!(options->float_mul_add{sz} & nir_float_muladd_support_prefers_split))'
    )
    late_optimizations.extend([
