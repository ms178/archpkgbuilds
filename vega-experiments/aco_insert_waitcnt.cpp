...
namespace aco {
      ...
      namespace {
            ...
            /* ----------------  VMEM-half mask table (build-time)  ------------------- */
            namespace {
                  ...
                  build_mask_tbl(std::make_index_sequence<static_cast<std::size_t>(aco_opcode::num_opcodes)>{});

            } // anonymous namespace

            /* Fast branch-free replacement for the old switch. */
            static inline uint32_t
            get_vmem_mask(wait_ctx& ctx, Instruction* instr)
            {
                  if (ctx.program->dev.sram_ecc_enabled)
                        return 0xffffffffu;
                  return vmem_mask_tbl[static_cast<int>(instr->opcode)];
            }
