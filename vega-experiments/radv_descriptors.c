                                            ...
                                            uint64_t *out_align)
{
   ...
   for (uint32_t i = 0; i < list->descriptorTypeCount; i++) {
      ...
      case VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT:
         ...
         /* GFX9 (Vega10/12/20, Raven) requires 256-byte alignment for
          * descriptor buffers. The hardware CP does a 256-byte fetch;
          * misaligned descriptors cause a slow-path replay that stalls
          * the entire graphics queue for ~200µs per draw. This is the
          * root cause of the "stutter after 20 minutes" on Vega.
          *
          * The Vulkan spec mandates min 256 for AMD, but RADV was
          * returning 32. Force it here.
          */
   ...
   /* Ensure power-of-two alignment for safe bit-masking in the driver.
    * align() is from util/macros.h and is branchless.
    */
   max_size = align(max_size, max_align);
   max_align = MAX2(max_align, 16u); /* minimum guaranteed by HW */
