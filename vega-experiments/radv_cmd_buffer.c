                      ...
                      const VkImageSubresourceRange *range)
{
   ...
   if (src_flags & VK_ACCESS_2_COMMAND_PREPROCESS_WRITE_BIT_EXT) {
      /* DGC preprocess writes are buffer-side writes. On GFX9+ with coherent buffer L2 behavior,
       * writeback can be skipped similarly to other buffer-side source accesses.
       */
      wb_l2 |= !can_skip_buffer_l2_flushes(device);
   }
...
radv_bind_descriptor_heap(struct radv_cmd_buffer *cmd_buffer, uint32_t heap_idx, uint64_t address)
{
   ...
   const uint8_t heap_bit = (uint8_t)(1u << heap_idx);
   for (unsigned i = 0; i < MAX_BIND_POINTS; i++) {
      struct radv_descriptor_state *descriptors_state = &cmd_buffer->descriptors[i];

      /* Track heap validity for all bind points, but only mark heap state dirty for bind points
       * that currently have descriptor state active. This avoids unnecessary dirty traffic when
       * apps bind heaps early and only use one pipeline bind point in the hot path.
       */
      descriptors_state->valid_heaps |= heap_bit;
      if (descriptors_state->valid)
         descriptors_state->dirty_heaps |= heap_bit;
   }
}
