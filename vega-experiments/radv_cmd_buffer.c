...
radv_CmdBindPipeline(VkCommandBuffer commandBuffer, VkPipelineBindPoint pipelineBindPoint, VkPipeline _pipeline)
{
   ...
   case VK_PIPELINE_BIND_POINT_COMPUTE: {
      ...
      pipeline_changed = cmd_buffer->state.compute_pipeline != compute_pipeline;
      radv_bind_compute_pipeline(cmd_buffer, compute_pipeline);
      break;
   }
   case VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR: {
      struct radv_ray_tracing_pipeline *rt_pipeline = radv_pipeline_to_ray_tracing(pipeline);
      pipeline_changed = cmd_buffer->state.rt_pipeline != rt_pipeline;
      radv_bind_rt_pipeline(cmd_buffer, rt_pipeline);
      break;
   }
   case VK_PIPELINE_BIND_POINT_GRAPHICS: {
      struct radv_graphics_pipeline *graphics_pipeline = radv_pipeline_to_graphics(pipeline);
      pipeline_changed = cmd_buffer->state.graphics_pipeline != graphics_pipeline;
   ...
   if (pipeline_changed) {
      ...
      cmd_buffer->descriptors[vk_to_bind_point(pipelineBindPoint)].dynamic_offset_count =
...
radv_CmdExecuteCommands(VkCommandBuffer commandBuffer, uint32_t commandBufferCount, const VkCommandBuffer *pCmdBuffers)
{
   ...
   bool secondary_emitted_compute_pipeline = false;
   bool secondary_emitted_rt_pipeline = false;
   const struct radv_physical_device *pdev = radv_device_physical(device);
   ...
   for (uint32_t i = 0; i < commandBufferCount; i++) {
      ...
      secondary_emitted_compute_pipeline |= !!secondary->state.emitted_compute_pipeline;
      secondary_emitted_rt_pipeline |= !!secondary->state.emitted_rt_pipeline;
   ...
   primary->state.dirty |= RADV_CMD_DIRTY_GRAPHICS_PIPELINE | RADV_CMD_DIRTY_INDEX_BUFFER |
                           RADV_CMD_DIRTY_GUARDBAND | RADV_CMD_DIRTY_SHADER_QUERY | RADV_CMD_DIRTY_OCCLUSION_QUERY |
                           RADV_CMD_DIRTY_DB_SHADER_CONTROL | RADV_CMD_DIRTY_FRAGMENT_OUTPUT;
   if (secondary_emitted_compute_pipeline)
      primary->state.dirty |= RADV_CMD_DIRTY_COMPUTE_PIPELINE;
   if (secondary_emitted_rt_pipeline)
      primary->state.dirty |= RADV_CMD_DIRTY_RAY_TRACING_PIPELINE;
   radv_mark_descriptors_dirty(primary, VK_PIPELINE_BIND_POINT_GRAPHICS);
   if (secondary_emitted_compute_pipeline || secondary_emitted_rt_pipeline)
...
radv_validate_dynamic_states(struct radv_cmd_buffer *cmd_buffer, uint64_t dynamic_states)
{
   ...
   /* Accumulate into a register-local to avoid ~29 load-OR-store sequences
    * against cmd_buffer->state.dirty (Intel Optimization Manual §2.4.4.3:
    * store-forwarding adds ~4-5 cycles per dependent load).
    */
   uint64_t dirty = 0;
   ...
   /* Single store instead of ~29 individual |= to cmd_buffer->state.dirty.
    * No function called above reads state.dirty, so accumulating locally is safe.
    */
...
radv_before_trace_rays(struct radv_cmd_buffer *cmd_buffer, struct radv_ray_tracing_pipeline *pipeline)
{
   ...
   const bool pipeline_is_dirty =
      !!(cmd_buffer->state.dirty & RADV_CMD_DIRTY_RAY_TRACING_PIPELINE) &&
      pipeline != cmd_buffer->state.emitted_rt_pipeline;
