...
static void calculate_needed_lds_size(struct si_screen *sscreen, struct si_shader *shader)
{
   ...
   if (sscreen->info.gfx_level >= GFX9 && stage <= MESA_SHADER_GEOMETRY &&
       (shader->key.ge.as_ngg || stage == MESA_SHADER_GEOMETRY)) {
      unsigned size_in_dw = 0;
      if (shader->key.ge.as_ngg) {
         size_in_dw = shader->ngg.info.esgs_lds_size;
         if (stage == MESA_SHADER_GEOMETRY) {
            size_in_dw += shader->ngg.info.ngg_out_lds_size;
         }
      } else if (stage == MESA_SHADER_GEOMETRY) {
         /*
          * For legacy GS, calculate a tighter LDS size based on the number
          * of invocations, not just the maximum possible vertex count.
          * This is a significant occupancy win for GS with low invocation counts.
          * The previous_stage_sel must be present for a separate GS part.
          */
         struct si_shader_selector *es_sel = shader->previous_stage_sel;
         assert(es_sel);
         ac_legacy_gs_compute_subgroup_info(shader->selector->info.base.gs.input_primitive,
                                            shader->selector->info.base.gs.vertices_out,
                                            shader->selector->info.base.gs.invocations,
                                            es_sel->info.esgs_vertex_stride,
                                            &shader->gs_info);
         size_in_dw = shader->gs_info.esgs_lds_size;
      }
   ...
   unsigned lds_size_in_bytes = shader->config.lds_size * get_lds_granularity(sscreen, stage);
   unsigned lds_hw_limit = (sscreen->info.gfx_level == GFX6 ? 32 : 64) * 1024;
   assert(lds_size_in_bytes <= lds_hw_limit);
}
...
void si_calculate_max_simd_waves(struct si_shader *shader)
{
   struct si_screen *sscreen = shader->selector->screen;
   struct ac_shader_config *conf = &shader->config;
   gl_shader_stage stage = shader->selector->stage;
   unsigned lds_increment = get_lds_granularity(sscreen, stage);
   unsigned lds_per_wave = 0;
   unsigned max_simd_waves;

   max_simd_waves = sscreen->info.max_waves_per_simd;

   /* Compute LDS usage per wave. */
   switch (stage) {
   case MESA_SHADER_FRAGMENT: {
      /*
       * GFX9+ hardware interpolation for non-flat inputs uses LDS space.
       * Instead of a pessimistic worst-case calculation, compute a more
       * precise LDS size based on which inputs are actually interpolated.
       * This is critical for occupancy. We iterate the final post-prolog
       * list of PS inputs from the variant info.
       *
       * Each non-flat interpolated input uses 48 bytes of LDS (a vec4's worth).
       */
      unsigned interp_lds_bytes = 0;
      if (sscreen->info.gfx_level >= GFX9) {
         for (unsigned i = 0; i < shader->info.num_ps_inputs; i++) {
            if (shader->info.ps_inputs[i].interpolate != INTERP_MODE_FLAT) {
               interp_lds_bytes += 48;
            }
         }
      }
      lds_per_wave = conf->lds_size * lds_increment + align(interp_lds_bytes, lds_increment);
      break;
   }
   case MESA_SHADER_COMPUTE: {
      unsigned max_workgroup_size = si_get_max_workgroup_size(shader);
      if (max_workgroup_size > shader->wave_size) {
...
static void run_late_optimization_and_lowering_passes(struct si_nir_shader_ctx *ctx)
{
   ...
   /*
    * For compute shaders, if the workgroup size is guaranteed to be less than
    * or equal to the wave size, all barriers are no-ops and can be removed.
    * This is a safe optimization as si_get_max_workgroup_size gives us a
    * guaranteed upper bound based on static shader properties.
    */
   if (nir->info.stage == MESA_SHADER_COMPUTE) {
      if (si_get_max_workgroup_size(shader) <= shader->wave_size) {
         bool removed_barriers = false;
   ...
   /*
    * These final passes improve instruction scheduling for memory access.
    * First, group all independent loads together to create larger contiguous
    * blocks of loads. This is more aggressive than only grouping loads from
    * the same resource.
    * Second, reorder the loads within those blocks. Loads whose results are
    * needed later are scheduled earlier, maximizing the latency-hiding
    * window for the entire group. This is highly effective on GFX9+.
    */
   NIR_PASS(_, nir, nir_group_loads, nir_group_all, 200);
