...
radv_sdma_pixel_offset_to_blocks(const VkOffset3D offset, const unsigned blk_w, const unsigned blk_h)
{
    ...
    * Critical for Vega 64 (SDMA 4.0) packet correctness.
    */
...
struct radv_sdma_surf
radv_sdma_get_surf(struct radv_cmd_buffer *cmd_buffer, const struct radv_image *const image,
                   VkImageLayout image_layout, const VkImageSubresourceLayers subresource, const VkOffset3D offset)
{
                                      ...
                                      const VkExtent3D pix_extent)
{
   ...
   /* Construct unified ac_sdma_surf for linear surfaces */
   const struct ac_sdma_surf ac_src = {
      .va = src->va,
      .format = radv_format_to_pipe_format(src->aspect_format),
      .bpp = src->bpp,
      .offset = { (uint32_t)src_off.x, (uint32_t)src_off.y, (uint32_t)src_off.z },
      .pitch = radv_sdma_pixels_to_blocks(src->pitch, src->blk_w),
      .slice_pitch = radv_sdma_pixel_area_to_blocks(src->slice_pitch, src->blk_w, src->blk_h),
   };

   const struct ac_sdma_surf ac_dst = {
      .va = dst->va,
      .format = radv_format_to_pipe_format(dst->aspect_format),
      .bpp = dst->bpp,
      .offset = { (uint32_t)dst_off.x, (uint32_t)dst_off.y, (uint32_t)dst_off.z },
      .pitch = radv_sdma_pixels_to_blocks(dst->pitch, dst->blk_w),
      .slice_pitch = radv_sdma_pixel_area_to_blocks(dst->slice_pitch, dst->blk_w, dst->blk_h),
   };

   radeon_check_space(device->ws, cs->b, 13);
   ac_emit_sdma_copy_linear_sub_window(cs->b, pdev->info.sdma_ip_version, &ac_src, &ac_dst,
                                     ...
                                     const VkExtent3D pix_extent, const bool detile)
{
   ...
   const struct ac_sdma_surf ac_linear = {
      .va = linear->va,
      .format = radv_format_to_pipe_format(linear->aspect_format),
      .offset = { (uint32_t)linear_off.x, (uint32_t)linear_off.y, (uint32_t)linear_off.z },
      .pitch = linear_pitch,
      .slice_pitch = linear_slice_pitch,
   };

   const struct ac_sdma_surf ac_tiled = {
      .surf = tiled->surf,
      .va = tiled->va,
      .format = radv_format_to_pipe_format(tiled->aspect_format),
      .bpp = tiled->bpp,
      .offset = { (uint32_t)tiled_off.x, (uint32_t)tiled_off.y, (uint32_t)tiled_off.z },
      .extent = { tiled_ext.width, tiled_ext.height, tiled_ext.depth },
   ...
   ac_emit_sdma_copy_tiled_sub_window(cs->b, &pdev->info, &ac_linear, &ac_tiled, detile,
                                   ...
                                   const VkExtent3D px_extent)
{
   ...
   const struct ac_sdma_surf ac_src = {
      ...
      .bpp = src->bpp,
      .offset = { (uint32_t)src_off.x, (uint32_t)src_off.y, (uint32_t)src_off.z },
      .extent = { src_ext.width, src_ext.height, src_ext.depth },
   ...
   const struct ac_sdma_surf ac_dst = {
      ...
      .bpp = dst->bpp,
      .offset = { (uint32_t)dst_off.x, (uint32_t)dst_off.y, (uint32_t)dst_off.z },
      .extent = { dst_ext.width, dst_ext.height, dst_ext.depth },
   ...
   ac_emit_sdma_copy_t2t_sub_window(cs->b, &pdev->info, &ac_src, &ac_dst,
                                    ext.width, ext.height, ext.depth);
}
                                ...
                                const struct radv_sdma_surf *dst, const VkExtent3D extent)
{
   ...
   const bool needs_3d_alignment = src->is_3d && (ver >= SDMA_7_0 ||
                                                  (src->surf->micro_tile_mode == RADEON_MICRO_MODE_DISPLAY ||
