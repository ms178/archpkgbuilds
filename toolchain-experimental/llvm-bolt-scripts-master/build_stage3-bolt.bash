...
LD_PRELOAD=/usr/lib/libmimalloc.so ${BOLTPATH}/perf2bolt ${CPATH}/lld \
    -p ${TOPLEV}/perf.data \
    -o ${TOPLEV}/lld.fdata || (echo "Could not convert perf-data to bolt for lld"; exit 1)

echo "Optimizing Clang with the generated profile"

if [ -f "${TOPLEV}/clang-20.fdata" ]; then
    ...
    echo "Profile data for Clang not found. Skipping optimization."
fi
...
if [ -f "${TOPLEV}/lld.fdata" ]; then
    ...
    echo "Profile data for LLD not found. Skipping optimization."
fi

# Move the original lld binary and replace it with the bolted version

echo "move bolted binary to lld"
if [ -f "${CPATH}/lld.bolt" ]; then
    mv ${CPATH}/lld ${CPATH}/lld.org
    mv ${CPATH}/lld.bolt ${CPATH}/lld
else
    echo "Bolted LLD binary not found. The original binary will remain unchanged."
fi
