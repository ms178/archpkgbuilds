...
static void cpa_flush_tlb(struct cpa_data *cpa)
{
	...
	if (cpa->force_flush_all) {
		flush_tlb_kernel_range(start, TLB_FLUSH_ALL);
		return;
	}
	...
	/*
	 * Array entries are not guaranteed contiguous. Flush only exact
	 * contiguous runs. For very large arrays, full flush tends to be
	 * cheaper than many range shootdowns.
	 */
	if (cpa->numpages > 64) {
		flush_tlb_kernel_range(start, TLB_FLUSH_ALL);
		return;
	...
	i = 0;
	while (i < cpa->numpages) {
		unsigned long addr = __cpa_addr(cpa, i);
		unsigned long run_start, run_end;
		...
		run_start = fix_addr(addr);
		run_end = run_start + PAGE_SIZE;
		i++;

		while (i < cpa->numpages) {
			unsigned long next_addr = __cpa_addr(cpa, i);
			unsigned long next;

			if (!next_addr)
				break;

			next = fix_addr(next_addr);
			if (next != run_end)
				break;
		...
		flush_tlb_kernel_range(run_start, run_end);
	}
}
...
static int cpa_process_alias(struct cpa_data *cpa)
{
	...
	/*
	 * No need to redo when the primary call touched the direct mapping.
	 */
	...
	if (!(within(vaddr, PAGE_OFFSET,
		...
		alias_cpa.flags &= ~(CPA_PAGES_ARRAY | CPA_ARRAY | CPA_FLUSHTLB);
		alias_cpa.curpage = 0;
		alias_cpa.force_flush_all = 0;
		...
		if (alias_cpa.flags & CPA_FLUSHTLB)
			alias_changed = true;
	}

#ifdef CONFIG_X86_64
	/*
	 * If the primary call didn't touch the high mapping already and
	 * the physical address is inside the kernel map, update highmap too.
	 */
