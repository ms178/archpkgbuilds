...
#define PREDICTION_FACTOR_DIVISOR \
	((u64)RESOLUTION * (u64)DECAY * (u64)NSEC_PER_USEC)
...
again:
	...
	filtered = (min_thresh != -1) || (max_thresh != (s64)UINT_MAX);

	if (likely(!filtered)) {
		for (i = 0; i < INTERVALS; i++) {
			unsigned int raw = intervals[i];

			/* Preserve legacy semantics: skip sentinel samples */
			if (unlikely(raw == UINT_MAX))
				continue;
	...
	if (likely(divisor == INTERVALS)) {
		avg >>= INTERVAL_SHIFT;
		variance >>= INTERVAL_SHIFT;
	} else {
		avg = div_u64(avg, divisor);
		variance = div_u64(variance, divisor);
	}
		       ...
		       bool *stop_tick)
{
	...
	struct cpuidle_state *states = drv->states;
	struct cpuidle_state_usage *usage = dev->states_usage;
	...
	if (unlikely(state_count <= 1 || latency_req == 0) ||
	    ((next_timer_ns < states[1].target_residency_ns ||
	      latency_req < states[1].exit_latency_ns) &&
	     !usage[0].disable)) {
		*stop_tick = !(states[0].flags & CPUIDLE_FLAG_POLLING);
		return 0;
	}

	tick_stopped = tick_nohz_tick_stopped();
	if (tick_stopped && predicted_ns < TICK_NSEC)
		predicted_ns = next_timer_ns;

	idx = -1;
	for (i = 0; i < state_count; i++) {
		struct cpuidle_state *s = &states[i];

		if (unlikely(usage[i].disable))
			continue;
		...
		if ((states[idx].flags & CPUIDLE_FLAG_POLLING) &&
		...
		if (!tick_stopped) {
			predicted_ns = states[idx].target_residency_ns;
			break;
		}

		if (states[idx].target_residency_ns < TICK_NSEC &&
		    s->target_residency_ns <= delta_tick)
			idx = i;

		/* Continue scanning deeper states */
	}
	...
	if (((states[idx].flags & CPUIDLE_FLAG_POLLING) ||
	     predicted_ns < TICK_NSEC) && !tick_stopped) {
		*stop_tick = false;

		if (idx > 0 &&
		    states[idx].target_residency_ns > delta_tick) {
			for (i = idx - 1; i >= 0; i--) {
				if (unlikely(usage[i].disable))
					continue;

				idx = i;
				if (states[i].target_residency_ns <=
				    delta_tick)
					break;
			}
		}
...
static void menu_update(struct cpuidle_driver *drv, struct cpuidle_device *dev)
{
	...
	u64 measured_ns;
	unsigned int *correction_factor = data->correction_factor;
	...
	new_factor = correction_factor[bucket];
	...
	correction_factor[bucket] = new_factor;
