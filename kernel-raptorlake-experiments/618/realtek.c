...
static void alc_update_headset_mode(struct hda_codec *codec)
{
	...
	/* Defensive guards for incomplete/odd BIOS pin configurations */
	if (!hp_pin || !spec->gen.input_mux.num_items) {
		snd_hda_gen_update_outputs(codec);
		return;
	}

	if (spec->gen.cur_mux[0] < spec->gen.input_mux.num_items)
		mux_pin = spec->gen.imux_pins[spec->gen.cur_mux[0]];

	if (!snd_hda_jack_detect(codec, hp_pin))
		new_headset_mode = ALC_HEADSET_MODE_UNPLUGGED;
	else if (spec->headset_mic_pin && mux_pin == spec->headset_mic_pin)
		new_headset_mode = ALC_HEADSET_MODE_HEADSET;
	else if (spec->headphone_mic_pin && mux_pin == spec->headphone_mic_pin)
	...
	case ALC_HEADSET_MODE_MIC:
		/* headphone_mic_pin is guaranteed non-zero by mode selection */
...
static void alc_probe_headset_mode(struct hda_codec *codec)
{
	...
	/* Find headset-capable mic pins first */
	for (i = 0; i < cfg->num_inputs; i++) {
	...
	/*
	 * Install hooks only when there is a meaningful topology to handle:
	 * at least one headset/headphone-mic pin and a valid hp pin.
	 */
	if (!alc_get_hp_pin(spec) ||
	    (!spec->headset_mic_pin && !spec->headphone_mic_pin))
		return;
