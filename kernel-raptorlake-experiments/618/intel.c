...
static void azx_init_pci(struct azx *chip)
{
	...
	bool snoop_on = azx_snoop(chip);

	/* Clear TCSEL QoS bits to avoid playback static on some codecs. */
	if (!(chip->driver_caps & AZX_DCAPS_NO_TCSEL)) {
		dev_dbg(chip->card->dev, "Clearing TCSEL\n");
		update_pci_byte(chip->pci, AZX_PCIREG_TCSEL, 0x07, 0);
	}

	/* ATI/AMD SB/Hudson snoop control */
	if (snoop_type == AZX_SNOOP_TYPE_ATI) {
		dev_dbg(chip->card->dev, "Setting ATI snoop: %d\n", snoop_on);
		update_pci_byte(chip->pci, ATI_SB450_HDAUDIO_MISC_CNTR2_ADDR, 0x07,
				snoop_on ? ATI_SB450_HDAUDIO_ENABLE_SNOOP : 0);
	}

	/* NVIDIA coherence/snoop related bits */
	if (snoop_type == AZX_SNOOP_TYPE_NVIDIA) {
		dev_dbg(chip->card->dev, "Setting Nvidia snoop: %d\n", snoop_on);
		update_pci_byte(chip->pci, NVIDIA_HDA_TRANSREG_ADDR, 0x0f,
				NVIDIA_HDA_ENABLE_COHBITS);
		update_pci_byte(chip->pci, NVIDIA_HDA_ISTRM_COH, 0x01,
				NVIDIA_HDA_ENABLE_COHBIT);
		update_pci_byte(chip->pci, NVIDIA_HDA_OSTRM_COH, 0x01,
				NVIDIA_HDA_ENABLE_COHBIT);
	}

	/* SCH/PCH snoop switch */
	if (snoop_type == AZX_SNOOP_TYPE_SCH) {
		unsigned short snoop;

		pci_read_config_word(chip->pci, INTEL_SCH_HDA_DEVC, &snoop);
		if ((!snoop_on && !(snoop & INTEL_SCH_HDA_DEVC_NOSNOOP)) ||
		    (snoop_on && (snoop & INTEL_SCH_HDA_DEVC_NOSNOOP))) {
...
static void bxt_reduce_dma_latency(struct azx *chip)
{
	...
	/* Clear DMA FIFO threshold control bits [21:20] to request DMA earlier. */
	val &= ~(0x3U << 20);
...
static int intel_ml_lctl_set_power(struct azx *chip, int state)
{
	...
	val &= ~AZX_ML_LCTL_SPA;
	val |= (u32)state << AZX_ML_LCTL_SPA_SHIFT;
	writel(val, bus->mlcap + AZX_ML_BASE + AZX_REG_ML_LCTL);

	cpa = (u32)state << AZX_ML_LCTL_CPA_SHIFT;

	while (timeout--) {
		val = readl(bus->mlcap + AZX_ML_BASE + AZX_REG_ML_LCTL);
		if ((val & AZX_ML_LCTL_CPA) == cpa)
			return 0;
		udelay(10);
	}
	...
	for (;;) {
		...
		list_for_each_entry(s, &bus->stream_list, list) {
			...
			if (ok > 0) {
				...
				/*
				 * Still pending:
				 *   ok == 0: delay handling requested
				 *   ok < 0 : too early/bogus for now
				 */
				pending++;
			}
		}
...
static void azx_clear_irq_pending(struct azx *chip)
{
	...
	/* Ensure pending worker is not running while we clear flags. */
...
static int azx_acquire_irq(struct azx *chip, int do_disconnect)
{
	...
	irq = pci_irq_vector(chip->pci, 0);
	ret = request_irq(irq, azx_interrupt, chip->msi ? 0 : IRQF_SHARED,
