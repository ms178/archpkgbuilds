...
static bool td_on_ring(struct xhci_td *td, struct xhci_ring *ring)
{
	...
	if (unlikely(!td))
		return false;
	seg = READ_ONCE(td->start_seg);
	if (unlikely(!seg))
		return false;

	first = READ_ONCE(ring->first_seg);
	if (unlikely(!first))
		return false;
	...
	/* Prefetch hides ~14-cycle L2 latency on Raptor Lake P-core */
	prefetch(first->next);
	xhci_for_each_ring_seg(first->next, seg) {
		if (seg == READ_ONCE(td->start_seg))
			return true;
	}
	return false;
}
...
int xhci_handshake(void __iomem *ptr, u32 mask, u32 done, u64 timeout_us)
{
	...
	ret = readl_poll_timeout_atomic(ptr, result,
					((result & mask) == done) ||
					(result == U32_MAX),
					0, (unsigned long)timeout_us);
	if (unlikely(result == U32_MAX)) /* card removed */
		return -ENODEV;
...
static int xhci_urb_enqueue(struct usb_hcd *hcd, struct urb *urb, gfp_t mem_flags)
{
	...
	/* Fast-path: avoid cache-line bounce on xhci->lock for dying host */
	if (unlikely(READ_ONCE(xhci->xhc_state) & XHCI_STATE_DYING))
		return -ESHUTDOWN;
	if (unlikely(!HCD_HW_ACCESSIBLE(hcd)))
		return -ESHUTDOWN;

	urb_priv = kmalloc(struct_size(urb_priv, td, num_tds), mem_flags);
	if (!urb_priv)
		return -ENOMEM;

	urb_priv->num_tds = num_tds;
	urb_priv->num_tds_done = 0;
	/* kmalloc avoids kzalloc's rep stos – init only what queue code expects */
	for (i = 0; i < num_tds; i++) {
		INIT_LIST_HEAD(&urb_priv->td[i].td_list);
		INIT_LIST_HEAD(&urb_priv->td[i].cancelled_td_list);
		urb_priv->td[i].status = 0;
		urb_priv->td[i].cancel_status = 0;
		urb_priv->td[i].urb = urb;
		urb_priv->td[i].start_seg = NULL;
		urb_priv->td[i].start_trb = NULL;
		urb_priv->td[i].end_seg = NULL;
		urb_priv->td[i].end_trb = NULL;
		urb_priv->td[i].bounce_seg = NULL;
		urb_priv->td[i].urb_length_set = false;
		urb_priv->td[i].error_mid_td = false;
	}
	WRITE_ONCE(urb->hcpriv, urb_priv);
	...
	slot_id = READ_ONCE(urb->dev->slot_id);
	...
	if (unlikely(*ep_state & (EP_GETTING_STREAMS | EP_GETTING_NO_STREAMS))) {
		xhci_warn(xhci, "WARN: Can't enqueue URB, ep in streams transition state %x\n",
			  *ep_state);
		ret = -EINVAL;
		goto free_priv;
	}
	if (unlikely(*ep_state & EP_SOFT_CLEAR_TOGGLE)) {
					  ...
					  struct usb_host_endpoint *ep)
{
	...
	spin_lock_irqsave(&xhci->lock, flags);
	udev = READ_ONCE(ep->hcpriv);
	if (unlikely(!udev))
		goto unlock;
	slot_id = READ_ONCE(udev->slot_id);
	if (unlikely(slot_id == 0 || slot_id >= MAX_HC_SLOTS))
		goto unlock;
	if (unlikely(!xhci->devs[slot_id]))
		goto unlock;
