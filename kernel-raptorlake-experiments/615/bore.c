...
static __always_inline u32 log2_u64_q24_8(u64 v)
{
	...
	u64 shifted = v << (63 - i);
	u8 f = (u8)(shifted >> 55);
	/* Vega-inspired LUT for precision (16 entries now, scalar-friendly, better accuracy) */
	static const u8 log_frac_lut[16] = {0, 13, 26, 38, 51, 63, 75, 87, 99, 111, 122, 134, 145, 156, 167, 178};
	f = log_frac_lut[f >> 4];
	return (i << 8) | f;
}
...
binary_smooth(u32 new_val, u32 old_val, enum x86_topology_cpu_type ctype)
{
	...
	u32 abs_inc = abs(increment);
	u32 shift = (increment >= 0) ? shift_long : shift_short;
	u32 delta = abs_inc >> shift;
	u32 result = (increment >= 0) ? old_val + delta : old_val - delta;
	return result;
}
...
struct bore_penalty_cache {
	...
	u64 last_burst_time;
	u32 last_penalty;
	int last_cpu; /* To invalidate on migration */
} ____cacheline_aligned_in_smp;
...
void update_burst_penalty(struct sched_entity *se)
{
	...
	if (cache->last_se == se && cache->last_burst_time == se->burst_time && cache->last_cpu == rq->cpu) {
		se->curr_burst_penalty = cache->last_penalty;
	} else {
		se->curr_burst_penalty =
		__calc_burst_penalty_fast(se->burst_time,
								  rq->cpu);
									...
									u64 now_ns, u64 clone_flags)
{
	...
	value = ancestor_for_inheritance->se.child_burst.value;
