...
#define BORE_UPDATE_MIN_NS         1000ULL
...
static void bore_build_penalty_param_for_cpu(int cpu)
{
    ...
    // Enhanced ITD bias: Prefer P-cores more aggressively if itd_pr high (probability 0.8 for common case)
    if (__builtin_expect_with_probability(itd_pr >= 512, 1, 0.8)) {
        scale = div_u64((u64)scale * 88, 100);
    } else {
        scale = div_u64((u64)scale * 95, 100);  // Mild boost for lower pri
    }
...
static __always_inline u32 log2_u64_q24_8(u64 v)
{
    ...
    // Expanded LUT for precision, scalar-optimized (16→32 entries, fits L1, per Intel ORM 2.4)
    static const u8 log_frac_lut[32] = {
        0, 6, 13, 19, 26, 32, 38, 44, 51, 57, 63, 69, 75, 81, 87, 93,
        99, 105, 111, 117, 122, 128, 134, 140, 145, 151, 156, 162, 167, 173, 178, 184
    };
    f = log_frac_lut[f >> 3];  // Finer granularity, reduces error <1%
...
binary_smooth(u32 new_val, u32 old_val, enum x86_topology_cpu_type ctype)
{
    ...
    // Branchless: Use conditional select for shift and sign
    bool is_positive = increment >= 0;
    u32 abs_inc = __builtin_abs(increment);
    u32 shift = is_positive ? shift_long : shift_short;
    u32 delta = abs_inc >> shift;
    u32 result = is_positive ? old_val + delta : old_val - delta;
    return result;
}
...
struct bore_penalty_cache {
	...
	u64 last_burst_time;
	u32 last_penalty;
	int last_cpu; /* last CPU id where cached value was computed */
	u8  gen;      /* bore_penalty_gen snapshot to detect param changes */
} ____cacheline_aligned_in_smp;
...
void update_burst_penalty(struct sched_entity *se)
{
    ...
    if (likely(cache->last_se == se &&
               cache->last_burst_time == se->burst_time &&
               cache->last_cpu == cpu &&
               cache->gen == current_gen)) {
        se->curr_burst_penalty = cache->last_penalty;
    } else {
        new_pen = static_call(bore_calc_penalty)(se->burst_time, ctype);
...
void update_curr_bore(u64 delta_exec, struct sched_entity *se)
{
	...
	/* Only recalc penalty when the additional delta is non-trivial,
	 * or when the total burst_time crosses a modest boundary.
	 *
	 * This reduces expensive pressure on update_burst_penalty during
	 * extremely fine-grained preemption/slice fragments (typical in some games).
	 */
	if (delta_exec >= BORE_UPDATE_MIN_NS || se->burst_time >= (BORE_UPDATE_MIN_NS * 8)) {
...
void restart_burst_rescale_deadline(struct sched_entity *se)
{
	...
	s64 vruntime_remaining = se->deadline - se->vruntime; /* signed */
	...
	if (new_eff_prio > old_eff_prio) { /* priority got worse */
		u64 abs_vrem = vruntime_remaining < 0 ? (u64)-vruntime_remaining : (u64)vruntime_remaining;
		u64 weight_units = __unscale_slice(abs_vrem, old_eff_prio);
		u64 vruntime_scaled_new = __scale_slice(weight_units, new_eff_prio);

		s64 vr_scaled_signed = (s64)vruntime_scaled_new;
		if (vruntime_remaining < 0)
			vr_scaled_signed = -vr_scaled_signed;

		se->deadline = se->vruntime + vr_scaled_signed;
	}
}
...
static void __update_child_burst_direct_locked(struct task_struct *p, u64 now_ns)
{
    ...
    // Manual unroll for up to 4 children (typical in games/compilers, per benchmarks)
    for (int i = 0; i < 4 && pos != head; ++i) {
        child_iter = list_entry_rcu(pos, struct task_struct, sibling);
													...
													u32 *accumulated_count, u32 *accumulated_sum)
{
	...
	for_each_child_bore(p, child_iter) {
		...
		/* Safe prefetch: Check for next-next to avoid deref null */
		if (child_iter->sibling.next && ((struct list_head *)child_iter->sibling.next)->next) {
			__builtin_prefetch(((struct list_head *)child_iter->sibling.next)->next, 0, 1);
		}
									...
									u64 now_ns, u64 clone_flags)
{
	...
	if (clone_flags & CLONE_PARENT)
		ancestor = rcu_dereference(parent_task->real_parent);

	if (unlikely(!ancestor))
		return 0;

	/* Walk up, using RCU dereferences for safety */
	for (;;) {
		struct task_struct *par = rcu_dereference(ancestor->real_parent);
		/* stop if root or if ancestor has > threshold children */
		if (!par || par == ancestor)
			break;
					  ...
					  u64 clone_flags, u64 now_ns)
{
	...
	se_new->child_burst.value = 0; se_new->child_burst.count = 0;
	se_new->group_burst.timestamp = 0;
	se_new->group_burst.value = 0; se_new->group_burst.count = 0;
