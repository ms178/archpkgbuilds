/*
 * Burst-Oriented Response Enhancer (BORE) CPU Scheduler - Core Logic
 *
 * Copyright (C) 2021-2025 Masahito Suzuki <firelzrd@gmail.com>
 *
 * This version merges the advanced hybrid CPU awareness features from a
 * heavily customized BORE implementation with the upstream BORE 6.0.0 codebase.
 * It combines the modern structure and API of the new version with the
 * sophisticated P/E core intelligence of the custom version.
 *
 * Key features of this merged implementation:
 *
 ...
 *
 * 2.  Refined Penalty Calculation and Smoothing:
 *     - A hybrid-aware penalty function based on the BORE 6.0.0 fixed-point
 *       math (`BURST_PENALTY_SHIFT`) but driven by per-core-type parameters.
 *     - Core-aware smoothing with distinct rates for penalty increase and
 *       decrease, adaptable to P-cores and E-cores.
 *
 * 3.  Robust Initialization and Sysctl Handling:
 *     - Comprehensive sysctl interface for all tunables, including the
 *       hybrid-specific ones.
 *     - Sysctl handlers that invalidate per-CPU parameter caches on change,
 *       ensuring tunables are applied live.
 *     - A pre-computed lookup table for penalty scaling percentages.
 *
 ...
 *       and cache-line alignment for per-CPU data.
 */
...
/*
 * <linux/cpu.h> briefly introduces a stub task_cpu_possible() for early-boot
 * code and then removes it again. The first inline in kernel/sched/sched.h
 * expects the real helper to exist, so (re-)install the canonical definition
 * here before we pull in sched.h.
 */
...
/*                          1.  Tunables                              */
/* ================================================================== */
#define BORE_DEF_SCHED_BORE                   1
#define BORE_DEF_BURST_EXCLUDE_KTHREADS       1
#define BORE_DEF_BURST_SMOOTHNESS_LONG        1  /* Shift value for slow increase */
#define BORE_DEF_BURST_SMOOTHNESS_SHORT       0  /* Shift value for fast decrease */
#define BORE_DEF_BURST_FORK_ATAVISTIC         2  /* 0=off, 1=direct parent, >=2 topological depth */
#define BORE_DEF_BURST_PARITY_THRESHOLD       2  /* Min cfs_rq->nr_queued to apply RUN_TO_PARITY */
#define BORE_DEF_BURST_PENALTY_OFFSET         24 /* Base offset for penalty calc */
#define BORE_DEF_BURST_PENALTY_SCALE          3180 /* Base scale factor */
#define BORE_DEF_BURST_CACHE_STOP_COUNT       64 /* Max children to scan for topological inheritance */
#define BORE_DEF_BURST_CACHE_LIFETIME         (75 * 1000 * 1000) /* 75 ms in ns */
#define BORE_DEF_DEADLINE_BOOST_MASK          (ENQUEUE_INITIAL | ENQUEUE_WAKEUP) /* Event mask */
...
DEF_U8(sched_bore,                    BORE_DEF_SCHED_BORE);
DEF_U8(sched_burst_exclude_kthreads,  BORE_DEF_BURST_EXCLUDE_KTHREADS);
DEF_U8(sched_burst_smoothness_long,   BORE_DEF_BURST_SMOOTHNESS_LONG);
DEF_U8(sched_burst_smoothness_short,  BORE_DEF_BURST_SMOOTHNESS_SHORT);
DEF_U8(sched_burst_fork_atavistic,    BORE_DEF_BURST_FORK_ATAVISTIC);
DEF_U8(sched_burst_parity_threshold,  BORE_DEF_BURST_PARITY_THRESHOLD);
DEF_U8(sched_burst_penalty_offset,    BORE_DEF_BURST_PENALTY_OFFSET);

DEF_U32(sched_burst_penalty_scale,     BORE_DEF_BURST_PENALTY_SCALE);
DEF_U32(sched_burst_cache_stop_count,  BORE_DEF_BURST_CACHE_STOP_COUNT);
DEF_U32(sched_burst_cache_lifetime,    BORE_DEF_BURST_CACHE_LIFETIME);
DEF_U32(sched_deadline_boost_mask,     BORE_DEF_DEADLINE_BOOST_MASK);

/* Hybrid Core-Aware Tunables */
...
DEF_U8(sched_burst_smoothness_long_p, BORE_DEF_BURST_SMOOTHNESS_LONG);
DEF_U8(sched_burst_smoothness_short_p, BORE_DEF_BURST_SMOOTHNESS_SHORT);
DEF_U8(sched_burst_smoothness_long_e, BORE_DEF_BURST_SMOOTHNESS_LONG);
DEF_U8(sched_burst_smoothness_short_e, BORE_DEF_BURST_SMOOTHNESS_SHORT);
...
/* ================================================================== */
DEFINE_PER_CPU(enum x86_topology_cpu_type, bore_cpu_type) ____cacheline_aligned_in_smp = TOPO_CPU_TYPE_UNKNOWN;

static __always_inline enum x86_topology_cpu_type bore_get_cpu_type(int cpu)
{
		...
		if (ct == TOPO_CPU_TYPE_EFFICIENCY) {
			...
			if (sched_burst_penalty_ecore_scale_pct > 100) {
				u8 adj = min_t(u8, MAX_ECORE_OFFSET_ADJUST,
							   (sched_burst_penalty_ecore_scale_pct - 100) / ECORE_OFFSET_ADJ_DIV);
...
static int __init bore_topology_init(void)
{
	...
	ret_hp = cpuhp_setup_state_nocalls(CPUHP_AP_ONLINE_DYN, "sched/bore:online",
									   bore_cpu_online_cb, NULL);
	if (ret_hp < 0) {
...
DEFINE_STATIC_CALL(bore_calc_penalty, __calc_burst_penalty);
...
/* ================================================================== */
static __always_inline u32 binary_smooth(u32 new_val, u32 old_val, int cpu)
{
	...
	if (sched_burst_core_aware_smoothing && static_branch_unlikely(&bore_core_aware_key)) {
		enum x86_topology_cpu_type ctype = bore_get_cpu_type(cpu);
...
void update_burst_score(struct sched_entity *se)
{
	...
	if (!((p->flags & PF_KTHREAD) && sched_burst_exclude_kthreads)) {
		new_score = se->burst_penalty >> BURST_PENALTY_SHIFT;
	}
	...
	if (static_branch_unlikely(&bore_core_aware_key) &&
		...
		new_score = min_t(u8, NICE_WIDTH - 1, new_score + sched_burst_pcore_hog_penalty_add);
		}
		#endif
...
void update_curr_bore(u64 delta_exec, struct sched_entity *se)
{
	...
	se->curr_burst_penalty = static_call(bore_calc_penalty)(se->burst_time, rq->cpu);

	/* Penalty increases immediately, but decreases are smoothed via restart_burst */
...
static void revolve_burst_penalty_state(struct sched_entity *se)
{
	...
	se->prev_burst_penalty = binary_smooth(se->curr_burst_penalty, se->prev_burst_penalty, rq->cpu);
	se->burst_time         = 0;
...
{
	revolve_burst_penalty_state(se);
...
void restart_burst_rescale_deadline(struct sched_entity *se)
{
	...
	if (new_eff_prio > old_eff_prio) { /* Priority got worse (higher prio value) */
...
static void update_burst_cache_locked(struct sched_burst_cache *bc,
									  ...
									  u32 children_count, u64 children_penalty_sum, u64 now_ns)
{
	u32 avg_child_penalty = children_count ? (children_penalty_sum / children_count) : 0;
...
static void __update_child_burst_topological_locked(struct task_struct *p, u64 now_ns,
													u32 recursion_depth,
													u32 *accumulated_count, u64 *accumulated_sum)
{
	u32 current_level_direct_children_count = 0;
	u64 current_level_sum_of_penalties = 0;
	...
	list_for_each_entry_rcu(child_iter, &p->children, sibling) {
		...
		unsigned long flags;
		spin_lock_irqsave(&desc_cache->lock, flags);
		if (!burst_cache_expired(desc_cache, now_ns)) {
			current_level_direct_children_count += desc_cache->count;
			current_level_sum_of_penalties += (u64)desc_cache->value * desc_cache->count;
		} else {
		...
		spin_unlock_irqrestore(&desc_cache->lock, flags);
...
static u32 inherit_burst_direct(struct task_struct *parent_task, u64 now_ns, u64 clone_flags)
{
	struct task_struct *target = parent_task;
	struct sched_burst_cache *bc;
	unsigned long flags;
	u32 penalty = 0;

	if (clone_flags & CLONE_PARENT) {
		target = rcu_dereference(parent_task->real_parent);
	}
	if (unlikely(!target)) return 0;

	bc = &target->se.child_burst;
	spin_lock_irqsave(&bc->lock, flags);
	if (burst_cache_expired(bc, now_ns)) {
...
static u32 inherit_burst_topological(struct task_struct *parent_task, u64 now_ns, u64 clone_flags)
{
	struct task_struct *ancestor = parent_task;
	u32 child_count_thresh = (clone_flags & CLONE_PARENT) ? 1 : 0;
	struct sched_burst_cache *bc;
	unsigned long flags;
	u32 penalty = 0;
	u32 dummy_count = 0;
	...
	if (clone_flags & CLONE_PARENT) {
		...
		if (unlikely(!ancestor)) return 0;
	}

	while (ancestor->real_parent != ancestor && ancestor->real_parent != NULL) {
		if (count_children_upto2_rcu(&ancestor->children) > child_count_thresh) {
			break;
		}
		ancestor = rcu_dereference(ancestor->real_parent);
		if (unlikely(!ancestor)) return 0;
		child_count_thresh = 1;
	}
	if (unlikely(!ancestor)) return 0;

	bc = &ancestor->se.child_burst;
	spin_lock_irqsave(&bc->lock, flags);
	if (burst_cache_expired(bc, now_ns)) {
		u32 depth = (sched_burst_fork_atavistic > 1) ? (sched_burst_fork_atavistic - 1) : 0;
		__update_child_burst_topological_locked(ancestor, now_ns, depth, &dummy_count, &dummy_sum);
	}
	penalty = bc->value;
	spin_unlock_irqrestore(&bc->lock, flags);
	return penalty;
...
static void __update_tg_burst_locked(struct task_struct *group_leader, u64 now_ns)
{
	...
	update_burst_cache_locked(&group_leader->se.group_burst, group_leader,
							  thread_count, sum_penalties, now_ns);
}

static u32 inherit_burst_tg(struct task_struct *parent_task, u64 now_ns)
{
	struct task_struct *leader = rcu_dereference(parent_task->group_leader);
	struct sched_burst_cache *bc;
	unsigned long flags;
	u32 penalty = 0;
	...
	bc = &leader->se.group_burst;
	spin_lock_irqsave(&bc->lock, flags);
	if (burst_cache_expired(bc, now_ns)) {
		__update_tg_burst_locked(leader, now_ns);
	}
	penalty = bc->value;
	spin_unlock_irqrestore(&bc->lock, flags);
	return penalty;
...
void sched_clone_bore(struct task_struct *p, struct task_struct *parent, u64 clone_flags, u64 now)
{
	...
	se->child_burst.value = 0; se->child_burst.count = 0;
	se->group_burst.timestamp = 0;
	se->group_burst.value = 0; se->group_burst.count = 0;
	...
	if (clone_flags & CLONE_THREAD) {
		inherited_penalty = inherit_burst_tg(parent, now);
	} else {
		if (sched_burst_fork_atavistic == 0) {
			inherited_penalty = 0;
		} else if (sched_burst_fork_atavistic == 1) {
			inherited_penalty = inherit_burst_direct(parent, now, clone_flags);
		} else {
			inherited_penalty = inherit_burst_topological(parent, now, clone_flags);
		}
	}
	rcu_read_unlock();
...
static void reset_all_task_weights_for_bore_toggle(void)
{
	...
	for_each_process(g) {
		for_each_thread(g, t) {
			if (!task_is_bore_eligible(t) || !tryget_task_struct(t)) {
				continue;
			}
...
static const int bore_sysctl_val_scale_max    = BORE_DEF_BURST_PENALTY_SCALE * 4;
static const int bore_sysctl_val_pct_max      = 200;
static const int bore_sysctl_val_stopcnt_max  = 4096;

static int sched_bore_toggle_sysctl_handler(const struct ctl_table *table, int write,
...
static int bore_u8_sysctl_gen_handler(const struct ctl_table *table, int write,
									  void *buffer, size_t *lenp, loff_t *ppos)
{
...
static int bore_uint_sysctl_gen_handler(const struct ctl_table *table, int write,
										void *buffer, size_t *lenp, loff_t *ppos)
{
...
{
	pr_info("BORE v%s (Hybrid Edition) initialising (HZ=%d)\n", SCHED_BORE_VERSION, HZ);
	...
	if (is_intel_hybrid()) {
		...
		sched_burst_parity_threshold += 2;
		sched_burst_smoothness_short = max_t(u8, 1, BORE_DEF_BURST_SMOOTHNESS_SHORT + 1);
...
BORE_SCHED_STUB(update_curr_bore, u64 delta_exec, struct sched_entity *se);
...
void sched_clone_bore(struct task_struct *p, struct task_struct *parent,
					  ...
					  #ifdef CONFIG_SYSCTL
					  late_initcall_sync(bore_sysctl_init);
					  #endif /* CONFIG_SYSCTL */
