...
static void wakeup_softirqd(void)
{
	...
	if (likely(tsk)) {
		wake_up_process(tsk);
		/*
		 * If we're the idle task and deferring softirq processing
		 * to ksoftirqd, ensure we reschedule before attempting to
		 * stop the tick. Without this, a race exists where the idle
		 * path may try to stop the tick (via tick_nohz_idle_stop_tick)
		 * while softirqs are still pending, triggering spurious
		 * "NOHZ tick-stop error: local softirq work is pending"
		 * warnings.
		 *
		 * wake_up_process() may not set TIF_NEED_RESCHED if ksoftirqd
		 * is already runnable. By explicitly setting need_resched here,
		 * we ensure can_stop_idle_tick() fails at its need_resched()
		 * check, giving ksoftirqd a chance to process pending softirqs.
		 */
...
static inline void invoke_softirq(void)
{
	if (likely(!force_irqthreads() || !__this_cpu_read(ksoftirqd))) {
...
{
	if (likely(test_and_clear_wake_up_bit(TASKLET_STATE_SCHED, &t->state)))
		return true;
	...
	while (list) {
		...
		if (likely(tasklet_trylock(t))) {
			if (likely(!atomic_read(&t->count))) {
				if (likely(tasklet_clear_sched(t))) {
					if (t->use_callback) {
