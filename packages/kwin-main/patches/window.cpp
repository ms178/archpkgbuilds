...
void Window::startAutoRaise()
{
    ...
    m_autoRaiseTimer->stop();
    m_autoRaiseTimer->setInterval(options->autoRaiseInterval());
    m_autoRaiseTimer->start();
}
...
{
    if (partial) {
        const QRectF currentGeometry = moveResizeGeometry();
        area.setLeft(std::min(area.left() - currentGeometry.width() + 100, area.left()));
        area.setTop(std::min(area.top() - currentGeometry.height() + 100, area.top()));
        area.setRight(std::max(area.right() + currentGeometry.width() - 100, area.right()));
        area.setBottom(std::max(area.bottom() + currentGeometry.height() - 100, area.bottom()));
    }
    if (!partial) {
...
void Window::checkUnrestrictedInteractiveMoveResize()
{
    ...
    if (isInteractiveResize()) {
        ...
        if (!isUnrestrictedInteractiveMoveResize() && moveResizeGeom.top() < desktopArea.top()) {
            setUnrestrictedInteractiveMoveResize(true);
        }
    }
}
...
bool Window::performMousePressCommand(Options::MouseCommand cmd, const QPointF &globalPos)
{
    ...
    case Options::MouseUnrestrictedMove: {
        ...
        setInteractiveMoveOffset(QPointF(qreal(globalPos.x() - x()) / width(), qreal(globalPos.y() - y()) / height()));
    ...
    case Options::MouseUnrestrictedResize: {
        ...
        setInteractiveMoveResizeAnchor(globalPos);
        setInteractiveMoveResizeModifiers(Qt::KeyboardModifiers());
        const QPointF moveOffset = QPointF(globalPos.x() - x(), globalPos.y() - y());
...
bool Window::isActiveFullScreen() const
{
    ...
    const auto ac = workspace()->mostRecentlyActivatedWindow();
...
bool Window::processDecorationButtonPress(const QPointF &localPos, const QPointF &globalPos, Qt::MouseButton button, bool ignoreMenu)
{
    ...
    return !(com == Options::MouseRaise || com == Options::MouseOperationsMenu || com == Options::MouseActivateAndRaise || com == Options::MouseActivate || com == Options::MouseActivateRaiseAndPassClick || com == Options::MouseActivateAndPassClick || com == Options::MouseNothing);
}
...
{
    workspace()->updateFocusMousePosition(Cursors::self()->mouse()->pos());
...
void Window::sendToOutput(Output *newOutput)
{
    ...
    const QRectF oldScreenArea = ws->clientArea(MaximizeArea, this, moveResizeOutput());
    const QRectF screenArea = ws->clientArea(MaximizeArea, this, newOutput);

    if (requestedQuickTileMode() == QuickTileMode(QuickTileFlag::Custom)) {
        ws->tileManager(moveResizeOutput())->forgetWindow(this, nullptr);
    } else {
        Tile *newTile = ws->tileManager(newOutput)->quickTile(requestedQuickTileMode());
        if (newTile) {
            newTile->manage(this);
        }
    }
    ...
    auto tso = ws->ensureStackingOrder(transients());
...
void Window::checkWorkspacePosition(QRectF oldGeometry, const VirtualDesktop *oldDesktop)
{
    ...
    const QRect oldGeomTall = QRect(oldGeometry.x(), oldScreenArea.y(), oldGeometry.width(), oldScreenArea.height());
    const QRect oldGeomWide = QRect(oldScreenArea.x(), oldGeometry.y(), oldScreenArea.width(), oldGeometry.height());
    ...
    const QRect newGeomTall = QRect(newGeom.x(), screenArea.y(), newGeom.width(), screenArea.height());
    const QRect newGeomWide = QRect(screenArea.x(), newGeom.y(), screenArea.width(), newGeom.height());

    auto moveAreaFunc = workspace()->inRearrange() ? &Workspace::previousRestrictedMoveArea :
...
{
    const Output *out = m_moveResizeOutput;
    setNextTargetScale(out ? out->scale() : qreal(1));
}
...
{
    const Output *out = m_moveResizeOutput;
    setPreferredBufferTransform(out ? out->transform() : OutputTransform::Normal);
}
...
{
    const Output *out = m_moveResizeOutput;
    setPreferredColorDescription(out ? out->blendingColor() : ColorDescription::sRGB);
}
