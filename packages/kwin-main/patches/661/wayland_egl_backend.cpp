...
WaylandEglLayer::~WaylandEglLayer() = default;
...
std::optional<OutputLayerBeginFrameInfo> WaylandEglLayer::doBeginFrame()
{
    ...
    if (!m_swapchain || m_swapchain->size() != nativeSize) [[unlikely]] {
...
bool WaylandEglLayer::doEndFrame(const Region &, const Region &damagedDeviceRegion, OutputFrame *frame)
{
    m_query->end();
...
bool WaylandEglLayer::importScanoutBuffer(GraphicsBuffer *buffer, const std::shared_ptr<OutputFrame> &)
{
    if (!test()) {
        return false;
    }
...
std::optional<OutputLayerBeginFrameInfo> WaylandEglCursorLayer::doBeginFrame()
{
    ...
    if (!m_swapchain || m_swapchain->size() != bufferSize) [[unlikely]] {
        if (m_lastFormat.format != 0) {
            m_swapchain = EglSwapchain::create(m_backend->drmDevice()->allocator(),
                                               m_backend->openglContext(),
                                               bufferSize,
...
bool WaylandEglCursorLayer::doEndFrame(const Region &, const Region &, OutputFrame *frame)
{
    m_query->end();
    if (frame) {
        frame->addRenderTimeQuery(std::move(m_query));
