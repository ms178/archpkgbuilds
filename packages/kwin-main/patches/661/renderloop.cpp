...
void RenderLoopPrivate::updateVrrState() noexcept {
    ...
    if (canVrr) {
        ...
        if (active && output) {
            ...
            LogicalOutput *logical = ws->findOutput(output);
            const bool onOutput = logical && active->isOnOutput(logical);
...
bool RenderLoopPrivate::detectVrrOscillation() noexcept {
    if (modeSwitchHistoryCount_ < kOscillationThreshold) return false;
    const auto now = std::chrono::steady_clock::now();
...
bool RenderLoopPrivate::shouldSwitchMode(PresentationMode target) noexcept {
    ...
    if (oscillationCooldownCounter_ > 0) {
        ...
        if (oscillationCooldownCounter_ == 0) vrrOscillationLockout = false;
        return false;
    }
    if (vrrOscillationLockout) return false;
    if (target == pendingTargetMode_) {
        if (pendingModeCounter_ < std::numeric_limits<uint16_t>::max()) ++pendingModeCounter_;
    } else {
    ...
    if (pendingModeCounter_ >= threshold) {
        if (detectVrrOscillation()) return false;
        pendingModeCounter_ = 0;
        return true;
    }
    return false;
...
PresentationMode RenderLoopPrivate::selectPresentationMode() noexcept {
    if (!vrrEnabled || !vrrCapable) [[likely]] return PresentationMode::VSync;
    if (vrrMode == VrrMode::Never) return PresentationMode::VSync;
    const auto state = vrrStateCache_.getState();
    const bool fullscreenOnOutput = state.isOnOutput && state.isFullScreen;
    if (!fullscreenOnOutput) return PresentationMode::VSync;
    if (vrrMode == VrrMode::Always) return PresentationMode::AdaptiveSync;
    if (cadenceStability_ >= kCadenceStableThreshold) return PresentationMode::VSync;
    if (!state.valid) return PresentationMode::VSync;
    const auto hint = static_cast<PresentationModeHint>(state.hint);
    if (hint == PresentationModeHint::VSync) return PresentationMode::VSync;
    if (cadenceStability_ > kCadenceUnstableThreshold) {
        if (presentationMode == PresentationMode::VSync) return PresentationMode::VSync;
    }
    if (hint == PresentationModeHint::Async) return PresentationMode::AdaptiveAsync;
...
void RenderLoopPrivate::scheduleRepaint(std::chrono::nanoseconds lastTarget) {
    ...
    if (predNs <= 0) predNs = renderJournal.result().count();
    if (predNs <= 0) predNs = static_cast<int64_t>(vblankNs >> 1);
    ...
    if (presentationMode == PresentationMode::VSync) [[likely]] {
        ...
        uint64_t targetVblankU = ceilDivU64Reciprocal(static_cast<uint64_t>(nsFromLastPres), vblankNs, vblankIntervalReciprocal64, reciprocalShift64);
        ...
        if (compositeNs > tripleBufferEnterThresholdNs) {
            if (tripleBufferHysteresisCounter < kTripleBufferHysteresisLimit) ++tripleBufferHysteresisCounter;
            else wasTripleBuffering = true;
    ...
    if (compositeTimer.isActive() && safeAbs64(diffNs) < kTimerJitterFilterNs) return;
    int timerMs = delayNs < kNsPerMs ? 0 : static_cast<int>(std::clamp(delayNs / kNsPerMs, int64_t{1}, int64_t{kMaxTimerDelayMs}));
    if (compositeTimer.isActive() && scheduledTimerMs >= 0) {
        if (timerMs == scheduledTimerMs && safeAbs64(diffNs) < kNsPerMs) return;
    }
...
void RenderLoopPrivate::notifyFrameCompleted(std::chrono::nanoseconds timestamp, std::optional<RenderTimeSpan> renderTime, PresentationMode mode, OutputFrame *frame) {
    auto processCompletion = [this, timestamp, renderTime, mode, frame]() {
        consecutiveErrorCount = 0;
        if (s_debugEnabled && frame) [[unlikely]] {
            writeDebug(this, renderTime, frame->targetPageflipTime().time_since_epoch(), frame->refreshDuration(), frame->predictedRenderTime(), timestamp, mode);
        }
        if (pendingFrameCount > 0) --pendingFrameCount;
        ...
        if (isVrrMode(mode)) {
            if (starvationRecoveryCounter < kStarvationRecoveryFrames) ++starvationRecoveryCounter;
    ...
    if (q->thread() == QThread::currentThread()) [[likely]] processCompletion();
    else QMetaObject::invokeMethod(q, std::move(processCompletion), Qt::QueuedConnection);
}
...
void RenderLoop::scheduleRepaint(Item *item, OutputLayer *layer) {
    ...
    if (vrrActive) maxPending = (d->starvationRecoveryCounter >= kStarvationRecoveryFrames) ? 2 : 1;
    if (d->pendingFrameCount < maxPending && d->inhibitCount == 0) d->scheduleNextRepaint();
    else d->pendingReschedule = true;
}
...
bool RenderLoop::activeWindowControlsVrrRefreshRate() const {
    ...
    if (!active || !d->output) return false;
    LogicalOutput *logical = ws->findOutput(d->output);
    if (!logical) return false;
    SurfaceItem *surface = active->surfaceItem();
    return surface && surface->recursiveFrameTimeEstimation().transform([](const auto t) { return t <= std::chrono::nanoseconds(1'000'000'000) / 30; }).value_or(false) && active->frameGeometry().intersects(logical->geometryF());
}
