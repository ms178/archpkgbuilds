...
{
public:
    [[nodiscard]] qreal accumulate(PointerAxisEvent *event) noexcept
    {
        const qreal delta = event->deltaV120 != 0 ? event->deltaV120 / 120.0 : event->delta / 15.0;

        if ((m_scrollDistance < 0.0) != (delta < 0.0)) [[unlikely]] {
            m_scrollDistance = 0.0;
        }

        m_scrollDistance += delta;

        if (m_scrollDistance >= 1.0 || m_scrollDistance <= -1.0) [[likely]] {
            const qreal whole = std::trunc(m_scrollDistance);
            m_scrollDistance -= whole;
            return whole;
        }
...
{
    if (!Workspace::self()) [[unlikely]] {
        return nullptr;
    }

    const bool isScreenLocked = waylandServer() && waylandServer()->isScreenLocked();
    if (!isScreenLocked) [[likely]] {
        if (effects && effects->isMouseInterception()) [[unlikely]] {
            return nullptr;
        }
    }

    const QList<Window *> &stacking = Workspace::self()->stackingOrder();
    if (stacking.isEmpty()) [[unlikely]] {
        return nullptr;
    }
    ...
    for (auto it = stacking.rbegin(); it != stacking.rend(); ++it) {
        ...
        if (window->isDeleted()) [[unlikely]] {
            continue;
        }

        if (!window->isOnCurrentActivity() || !window->isOnCurrentDesktop()
            || window->isMinimized() || window->isHidden() || window->isHiddenByShowDesktop()) [[unlikely]] {
            continue;
        }

        if (!window->readyForPainting()) [[unlikely]] {
            continue;
        }

        if (isScreenLocked) [[unlikely]] {
            if (!window->isLockScreen() && !window->isInputMethod() && !window->isLockScreenOverlay()) {
                continue;
            }
        }

        if (window->hitTest(pos)) [[likely]] {
            return window;
        }
    }
