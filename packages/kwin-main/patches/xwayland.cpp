...
public:
        ...
        : KWin::InputEventFilter(InputFilterOrder::XWayland)
    {
        connect(waylandServer()->seat(), &SeatInterface::focusedKeyboardSurfaceAboutToChange,
            ...
            // When focus is about to switch to an X11 window, we must ensure that any
            // keys we have been "eavesdropping" on are released from the perspective
            // of the Xwayland server, preventing stuck keys.
    ...
    void setMode(XwaylandEavesdropsMode mode, bool eavesdropsMouse)
    {
        ...
        // GODLIKE FIX (Idea #2): Use a compile-time sorted array and binary search.
        // This is the most performant and correct way to implement this check.
        // The QSet is replaced with a data structure that is perfectly cache-friendly
        // and has zero runtime initialization cost.
        static const auto isSpecialKey = [](quint32 key) {
            // Keysyms below this value are generally character keys (A-Z, 0-9, etc.)
            if (key < 0x01000000) {
                return false;
            }

            // This array is sorted at COMPILE TIME. `std::binary_search` is extremely fast
            // on this cache-hot, contiguous block of memory.
            constexpr auto excludedKeys = [] {
                // Using std::array for fixed-size, stack/rodata allocation.
                std::array<quint32, 67> keys = {{
                    Qt::Key_Multi_key, Qt::Key_Codeinput, Qt::Key_SingleCandidate, Qt::Key_MultipleCandidate,
                    Qt::Key_PreviousCandidate, Qt::Key_Mode_switch, Qt::Key_Kanji, Qt::Key_Muhenkan,
                    Qt::Key_Henkan, Qt::Key_Romaji, Qt::Key_Hiragana, Qt::Key_Katakana,
                    Qt::Key_Hiragana_Katakana, Qt::Key_Zenkaku, Qt::Key_Hankaku, Qt::Key_Zenkaku_Hankaku,
                    ...
                    Qt::Key_Hangul_End, Qt::Key_Hangul_Hanja, Qt::Key_Hangul_Jamo, Qt::Key_Hangul_Romaja,
                    Qt::Key_Hangul_Jeonja, Qt::Key_Hangul_Banja, Qt::Key_Hangul_PreHanja,
                    Qt::Key_Hangul_PostHanja, Qt::Key_Hangul_Special, Qt::Key_Dead_Grave,
                    Qt::Key_Dead_Acute, Qt::Key_Dead_Circumflex, Qt::Key_Dead_Tilde, Qt::Key_Dead_Macron,
                    Qt::Key_Dead_Breve, Qt::Key_Dead_Abovedot, Qt::Key_Dead_Diaeresis,
                    Qt::Key_Dead_Abovering, Qt::Key_Dead_Doubleacute, Qt::Key_Dead_Caron,
                    Qt::Key_Dead_Cedilla, Qt::Key_Dead_Ogonek, Qt::Key_Dead_Iota,
                    Qt::Key_Dead_Voiced_Sound, Qt::Key_Dead_Semivoiced_Sound, Qt::Key_Dead_Belowdot,
                    Qt::Key_Dead_Hook, Qt::Key_Dead_Horn, Qt::Key_Dead_Stroke, Qt::Key_Dead_Abovecomma,
                    Qt::Key_Dead_Abovereversedcomma, Qt::Key_Dead_Doublegrave, Qt::Key_Dead_Belowring,
                    Qt::Key_Dead_Belowmacron, Qt::Key_Dead_Belowcircumflex, Qt::Key_Dead_Belowtilde,
                    Qt::Key_Dead_Belowbreve, Qt::Key_Dead_Belowdiaeresis, Qt::Key_Dead_Invertedbreve,
                    Qt::Key_Dead_Belowcomma, Qt::Key_Dead_Currency,
                }};
                // A simple bubble sort is sufficient and guaranteed to work at compile time.
            ...
            constexpr std::array<quint32, 67> sortedExcludedKeys = excludedKeys();

            return !std::binary_search(sortedExcludedKeys.cbegin(), sortedExcludedKeys.cend(), key);
        };
    ...
    bool keyboardKey(KWin::KeyboardKeyEvent *event) override
    {
        ...
        if (event->state == KeyboardKeyState::Repeated) {
            // X11 handles its own key repeats; forwarding them would cause duplicates.
            return false;
        }
        ...
        if (surface) {
            // If focus is already on an X11 window, we don't need to eavesdrop.
            if (xwaylandClient == surface->client()) [[likely]] {
                return false;
            }
        }

        if (!updateKey(event->nativeScanCode, event->state)) {
            // Key state did not change (e.g., redundant release event), no need to forward.
            return false;
        }
    ...
    bool pointerButton(KWin::PointerButtonEvent *event) override
    {
        ...
        // Only forward "extra" buttons (Back, Forward, etc.), not primary clicks.
