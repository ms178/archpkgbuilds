...
void X11Window::updateShapeRegion()
{
    ...
    if (Xcb::Extensions::self()->hasShape(m_client)) {
        auto cookie = xcb_shape_get_rectangles_unchecked(kwinApp()->x11Connection(), m_client, XCB_SHAPE_SK_BOUNDING);
        ...
        if (reply) {
            ...
            m_shapeRegion.reserve(rectCount);
