...
{
    return m_item && recursiveNeedsRepaint(m_item, this);
}
...
static double recursiveMaxHdrHeadroom(Item *rootItem)
{
    ...
    while (!stack.empty()) {
        ...
        if (refLuminance > 0.0) [[likely]] {
            const double max = color->maxHdrLuminance().value_or(refLuminance);
            const double headroom = max / refLuminance;
            if (headroom > globalMaxHeadroom) {
                globalMaxHeadroom = headroom;
            }
        }
