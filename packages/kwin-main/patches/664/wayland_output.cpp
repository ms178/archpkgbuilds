...
void WaylandOutput::handleFractionalScaleChanged(void *data, struct wp_fractional_scale_v1 *wp_fractional_scale_v1, uint32_t scale120)
{
    ...
    auto *const output = reinterpret_cast<WaylandOutput *>(data);
    output->m_pendingScale = static_cast<qreal>(scale120) / 120.0;
}
    ...
    , m_cursor(std::make_unique<WaylandCursor>(backend))
{
    ...
    if (auto manager = backend->display()->colorManager()) {
        const bool supportsMinFeatures =
            manager->supportsFeature(WP_COLOR_MANAGER_V1_FEATURE_PARAMETRIC) &&
        ...
        if (supportsMinFeatures) {
            ...
            connect(m_colorSurfaceFeedback.get(),
                    &ColorSurfaceFeedback::preferredColorChanged,
                    this,
                    &WaylandOutput::updateColor);
        }
    }
                            ...
                            uint32_t flags)
{
    ...
    const auto timestamp =
        std::chrono::seconds((uint64_t(tv_sec_hi) << 32U) | uint64_t(tv_sec_lo)) +
        std::chrono::nanoseconds(tv_nsec);

    uint32_t refreshRate = 60'000U;
    if (refresh != 0U) {
        const uint64_t mHz = 1'000'000'000'000ULL / static_cast<uint64_t>(refresh);
        refreshRate = mHz > 0xFFFF'FFFFULL ? 0xFFFF'FFFFU : static_cast<uint32_t>(mHz);
    }
...
bool WaylandOutput::present(const QList<OutputLayer *> &layersToUpdate, const std::shared_ptr<OutputFrame> &frame)
{
    ...
    if (!cursorLayers.empty()) {
        // Preserve original semantics to avoid pointer-lock regressions in games.
    ...
    if (!m_mapped) {
        // Keep original one-time mapping behavior.
        wl_buffer *const buffer =
            wp_single_pixel_buffer_manager_v1_create_u32_rgba_buffer(
                m_backend->display()->singlePixelManager(), 0, 0, 0, 0xFFFFFFFFU);
        m_surface->attachBuffer(buffer);
        m_mapped = true;
    }

    const qreal currentScale = scale();
    const qreal safeScale = currentScale > 0.0 ? currentScale : 1.0;
    const int logicalWidth =
        std::max(1, static_cast<int>(std::lround(static_cast<double>(modeSize().width()) / safeScale)));
    const int logicalHeight =
        std::max(1, static_cast<int>(std::lround(static_cast<double>(modeSize().height()) / safeScale)));

    m_viewport->setDestination(QSize(logicalWidth, logicalHeight));
    m_surface->setScale(1);

    // Commit subsurfaces before the main surface.
    for (OutputLayer *layer : layersToUpdate) {
        if (layer == nullptr || layer->type() == OutputLayerType::CursorOnly) {
            continue;
        }
    ...
    struct wp_presentation_feedback *const presentationFeedback =
        wp_presentation_feedback(m_backend->display()->presentationTime(), *m_surface);
    struct wl_callback *const frameCallback = wl_surface_frame(*m_surface);

    if (presentationFeedback == nullptr || frameCallback == nullptr) {
    ...
    FrameData frameData{frame, presentationFeedback, frameCallback};
...
void WaylandOutput::framePresented(std::chrono::nanoseconds timestamp, uint32_t refreshRate)
{
    ...
    const FrameData &frame = m_frames.front();
    if (frame.frameCallbackTime.has_value()) {
        auto margin = timestamp - frame.frameCallbackTime->time_since_epoch() + std::chrono::milliseconds(1);
        if (margin < std::chrono::nanoseconds::zero()) {
            margin = std::chrono::nanoseconds::zero();
        }
        m_renderLoop->setPresentationSafetyMargin(margin);
    }
...
{
    (void)allowedVrrDelay;
    return layer != nullptr && layer->type() == OutputLayerType::CursorOnly;
}
...
void WaylandOutput::handleConfigure(const QSize &size, XdgShellSurface::States states, quint32 serial)
{
    ...
    if (!m_configureThrottleTimer.isActive()) {
        const int refresh = m_state.currentMode ? m_state.currentMode->refreshRate() : static_cast<int>(m_refreshRate);
        const int safeRefresh = std::max(refresh, 1);
        const int intervalMs = std::max(1, 1'000'000 / safeRefresh);
...
void WaylandOutput::applyConfigure(const QSize &size, quint32 serial)
{
    ...
    const qreal appliedScale = m_pendingScale > 0.0 ? m_pendingScale : 1.0;
    const int scaledWidth =
        std::max(1, static_cast<int>(std::lround(static_cast<double>(size.width()) * appliedScale)));
    const int scaledHeight =
        std::max(1, static_cast<int>(std::lround(static_cast<double>(size.height()) * appliedScale)));

    const auto mode = std::make_shared<OutputMode>(QSize(scaledWidth, scaledHeight), m_refreshRate);

    State next = m_state;
    next.modes = {mode};
    next.currentMode = mode;
    next.scale = appliedScale;
    setState(next);
...
{
    if (!lock) {
        const bool surfaceWasLocked = (m_pointerLock != nullptr) && m_hasPointerLock;
        m_pointerLock.reset();
    ...
    auto *const constraints = m_backend->display()->pointerConstraints();
    if (constraints == nullptr || pointer == nullptr) {
        return;
    }

    Q_ASSERT(!m_pointerLock);
    m_pointerLock.reset(constraints->lockPointer(surface(), pointer, nullptr, PointerConstraints::LifeTime::OneShot));
    if (!m_pointerLock || !m_pointerLock->isValid()) {
        m_pointerLock.reset();
        return;
    }
