...
{
    return surface() ? surface()->client()->processId() : pid_t(-1);
}
...
void WaylandWindow::updateClientOutputs()
{
    ...
    Display *const disp = waylandServer()->display();
    surface()->setOutputs(disp->outputsIntersecting(rect),
                          disp->largestIntersectingOutput(rect));
}
