...
void ColorSurfaceV1::wp_color_management_surface_v1_set_image_description(Resource *resource, struct ::wl_resource *image_description, uint32_t render_intent)
{
    ...
    auto *descObj = ImageDescriptionV1::get(image_description);
    if (!descObj) {
        wl_resource_post_error(resource->handle, WL_DISPLAY_ERROR_INVALID_OBJECT, "invalid image_description object");
        return;
    }
    const auto &optDesc = descObj->description();
    ...
    const auto priv = SurfaceInterfacePrivate::get(m_surface);
    priv->pending->colorDescription = *optDesc;
...
constexpr double s_primaryScale = 1'000'000.0;
...
{
    return std::llround(left * s_primaryScale) == std::llround(right * s_primaryScale);
}
...
void ImageDescriptionV1::wp_image_description_v1_get_information(Resource *qtResource, uint32_t information)
{
    ...
    const auto roundPrimary = [](double f) -> uint32_t {
        const long long v = std::llround(f * s_primaryScale);
        const long long clamped = std::clamp<long long>(v, 0LL, 1'000'000LL);
    ...
    wp_image_description_info_v1_send_primaries(resource,
                                                roundPrimary(containerRed.x), roundPrimary(containerRed.y),
                                                roundPrimary(containerGreen.x), roundPrimary(containerGreen.y),
                                                roundPrimary(containerBlue.x), roundPrimary(containerBlue.y),
                                                roundPrimary(containerWhite.x), roundPrimary(containerWhite.y));
    ...
    wp_image_description_info_v1_send_target_primaries(resource,
                                                       roundPrimary(masterRed.x), roundPrimary(masterRed.y),
                                                       roundPrimary(masterGreen.x), roundPrimary(masterGreen.y),
                                                       roundPrimary(masterBlue.x), roundPrimary(masterBlue.y),
                                                       roundPrimary(masterWhite.x), roundPrimary(masterWhite.y));
