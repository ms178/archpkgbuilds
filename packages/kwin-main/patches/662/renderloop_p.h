...
public:
    struct State {
        ...
        uint8_t valid : 1;
        uint8_t reserved : 3;
        constexpr State() noexcept : hint(0), isOnOutput(0), isFullScreen(0), valid(0), reserved(0) {}
        [[nodiscard]] constexpr uint8_t toRaw() const noexcept { return std::bit_cast<uint8_t>(*this); }
        [[nodiscard]] static constexpr State fromRaw(uint8_t raw) noexcept { return std::bit_cast<State>(raw); }
    };
