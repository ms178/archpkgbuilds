...
DrmPipeline::Error DrmPipeline::present(const QList<OutputLayer *> &layersToUpdate, const std::shared_ptr<OutputFrame> &frame)
{
    ...
    for (OutputLayer *layer : layersToUpdate) {
        ...
        auto *const pipelineLayer = static_cast<DrmPipelineLayer *>(layer);
        DrmPlane *const plane = pipelineLayer->plane();
        if (!plane) [[unlikely]] {
    ...
    if (layerCount == 0) [[unlikely]] {
        ...
        DrmPipelineLayer *fallbackLayer = nullptr;
        ...
        if (!fallbackLayer) [[unlikely]] {
            qCWarning(KWIN_DRM) << "No enabled fallback layer";
            return Error::InvalidArguments;
        }

        DrmPlane *const fallbackPlane = fallbackLayer->plane();
    ...
    if (!partialUpdate->test()) [[unlikely]] {
        ...
        qCDebug(KWIN_DRM) << "Partial atomic present test failed! errno:" << savedErrno << strerror(savedErrno);
        return errnoToError();
    }

    m_next.needsModesetProperties = false;
...
DrmPipeline::Error DrmPipeline::prepareAtomicPresentation(DrmAtomicCommit *commit, const std::shared_ptr<OutputFrame> &)
{
    ...
    const ColorPipeline mergedPipeline = layerPipeline.isIdentity()
        ? crtcPipeline
...
bool DrmPipeline::presentAsync(OutputLayer *layer, std::optional<std::chrono::nanoseconds> allowedVrrDelay)
{
    ...
    auto *const drmLayer = static_cast<DrmPipelineLayer *>(layer);
    if (!drmLayer) [[unlikely]] {
        return false;
    }

    DrmPlane *const plane = drmLayer->plane();
    if (!plane) [[unlikely]] {
...
BackendOutput::RgbRange DrmPipeline::rgbRange() const noexcept
{
