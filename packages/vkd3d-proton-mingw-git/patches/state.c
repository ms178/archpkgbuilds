...
{
    /* Perfect branchless LUT. D3D12_SHADER_VISIBILITY_ALL == 0 */
    static const VkShaderStageFlags table[] = {
        VK_SHADER_STAGE_ALL,                    /* 0 = ALL */
        VK_SHADER_STAGE_VERTEX_BIT,             /* 1 = VERTEX */
    ...
    /* Clamp to prevent out-of-bounds (defensive, zero-cost on modern CPUs) */
...
enum vkd3d_shader_visibility vkd3d_shader_visibility_from_d3d12(D3D12_SHADER_VISIBILITY visibility)
{
    static const enum vkd3d_shader_visibility table[] = {
        VKD3D_SHADER_VISIBILITY_ALL,            /* 0 = ALL */
        ...
        struct d3d12_device *device, const D3D12_ROOT_SIGNATURE_DESC2 *desc)
{
    ...
    /* Global heap bindings - always present for robustness (very common path) */
    ...
    for (i = 0; i < desc->NumParameters; ++i)
    {
        ...
        switch (p->ParameterType)
        {
            case D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE:
                ...
                /* Cost model for table: 1 or 2 dwords depending on local RS (very hot path) */
        ...
        struct VkPushConstantRange *push_constant_range)
{
    ...
    /* Raw VA root descriptors first (alignment) - very common path */
    for (i = 0; i < desc->NumParameters; ++i)
    {
        const D3D12_ROOT_PARAMETER1 *p = &desc->pParameters[i];
