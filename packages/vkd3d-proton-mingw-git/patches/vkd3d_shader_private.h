...
{
    uint32_t limit_quirks = quirks & (VKD3D_SHADER_QUIRK_LIMIT_TESS_FACTORS_4 |
    ...
    /* Smallest limit wins if multiple are set. Branchless priority for CPU (Intel Optimization Manual §3.4). */
    if (limit_quirks & VKD3D_SHADER_QUIRK_LIMIT_TESS_FACTORS_4)
        return 4;
    if (limit_quirks & VKD3D_SHADER_QUIRK_LIMIT_TESS_FACTORS_8)
        return 8;
    if (limit_quirks & VKD3D_SHADER_QUIRK_LIMIT_TESS_FACTORS_12)
        return 12;
    if (limit_quirks & VKD3D_SHADER_QUIRK_LIMIT_TESS_FACTORS_16)
        return 16;
    if (limit_quirks & VKD3D_SHADER_QUIRK_LIMIT_TESS_FACTORS_32)
        return 32;
