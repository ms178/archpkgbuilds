...
namespace dxvk {
  ...
  VkResult DxvkCommandList::submit(
    ...
    for (size_t i = 0; i < m_cmdSubmissions.size(); i++) {
      ...
      if (isFirst) {
        ...
        for (size_t j = 0; j < m_waitSemaphores.size(); j++) {
      ...
      if (isLast) {
        ...
        for (size_t j = 0; j < m_signalSemaphores.size(); j++) {
  ...
  void DxvkCommandList::bindResourcesLegacy(
    ...
    if (pushDataSize && !pushDataBlock.isEmpty()) {
      ...
      size_t copySize = std::min(size_t(pushDataBlock.getSize()), std::min(dataCopy.size(), pushDataSize));
