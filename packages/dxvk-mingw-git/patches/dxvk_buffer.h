...
namespace dxvk {
  ...
  public:
    ...
    Rc<DxvkResourceAllocation> allocateStorage(DxvkLocalAllocationCache* cache) {
      ...
      VkBufferCreateInfo info = getBufferCreateInfo();
  ...
  public:
    ...
    : m_buffer(std::move(buffer)),
      m_offset(0),
      m_length(m_buffer != nullptr ? m_buffer->info().size : 0u) { }
    ...
    DxvkLegacyDescriptor getDescriptor() const {
      return m_buffer != nullptr
        ? m_buffer->getDescriptor(m_offset, m_length)
        : DxvkLegacyDescriptor();
    }
