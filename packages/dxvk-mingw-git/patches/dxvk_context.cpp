...
namespace dxvk {
  ...
  void DxvkContext::setViewports(
    ...
    auto rectEq = [] (const VkRect2D& a, const VkRect2D& b) {
    ...
    std::array<VkViewport, DxvkLimits::MaxNumViewports> newViewports;
    std::array<VkRect2D,   DxvkLimits::MaxNumViewports> newScissors;
    ...
    for (uint32_t i = 0; i < viewportCount; i++) {
      m_state.vp.viewports[i] = newViewports[i];
      m_state.vp.scissorRects[i] = newScissors[i];
    }
