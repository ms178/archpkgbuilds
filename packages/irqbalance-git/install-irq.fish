...
# C5: ban the whole managed CPU set, not just our modules.
#
# Two independent mechanisms, deliberately belt-and-braces:
#
#   IRQBALANCE_BANNED_CPULIST -- irqbalance will not place ANY irq on these
#     CPUs. This is the important one: --banmod alone only protects the listed
#     modules' IRQs, leaving every other device in the system free to migrate
#     onto the cores we just cleared.
#
#   --banmod=<drv> -- keeps our own devices out of irqbalance's database
#     entirely, so it never even considers moving what we pinned. Verified:
#     a banned IRQ is never "Added to database" and is reported as BANNED.
#
# The value is written as Environment= in a drop-in rather than to
# /etc/irqbalance.env on purpose. The upstream unit reads
#   EnvironmentFile=/usr/etc/default/irqbalance.env   (a path no package
#                                                      creates), and
#   EnvironmentFile=-${prefix}/etc/default/irqbalance (systemd rejects it:
#     "EnvironmentFile= path is not absolute, ignoring" -- this appears in
#     the journal on every boot).
# A drop-in Environment= line bypasses that whole broken path and cannot be
# silently ignored.
#
# Note on --banmod matching: irqbalance uses strstr(actual_module, pattern),
# i.e. a SUBSTRING match. Banning "ixgbe" also bans "ixgbevf" and "igb" also
# bans "igbvf". Both are same-family virtual functions of a card we are
# already managing, so the over-match is harmless here -- but it is why we do
# not ban very short or generic module names.
function write_irqbalance_dropin
    mkdir -p "$IRQBALANCE_DROPIN_DIR"
    set -l banned $POOL_NET $POOL_GPU $POOL_IO $POOL_MISC $POOL_GAME
    set -l banlist
    for drv in $EFF_DRIVERS
        # Refuse pathologically short patterns: a 2-char substring would
        # match half the module namespace.
        if test (string length -- "$drv") -ge 3
            set banlist $banlist "--banmod=$drv"
        else
            log_warn "skipping --banmod=$drv (too short for a substring match)"
        end
    end
    begin
        echo "[Service]"
        echo "# Generated by install-irq.fish v$SCRIPT_VERSION -- do not edit."
...
# Confirm at runtime that irqbalance actually respects the ban. If the daemon
# ever stops honouring it, the pinning guarantee is silently lost, so this is
# checked rather than assumed.
function verify_irqbalance_ban
    set -l banned (collapse_cpulist $POOL_NET $POOL_GPU $POOL_IO $POOL_MISC $POOL_GAME)
    if not type -q irqbalance
        log_debug "irqbalance not installed; ban check skipped"
        return 0
    end
    set -l out (env IRQBALANCE_BANNED_CPULIST="$banned" \
        timeout 30 irqbalance --oneshot --debug 2>&1 | string match -r '^Banned CPUs.*')
    if test -n "$out"
        log_info "irqbalance ban check: $out"
        return 0
    end
    log_warn "irqbalance did not report a banned CPU mask; pinning may be fought"
    return 1
end
